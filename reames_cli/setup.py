...
from reames_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from reames_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from reames_cli.curses_ui import curses_checklist
...
def _print_setup_summary(config: dict, hermes_home):
    ...
    if subscription_features.image_gen.managed_by_nous:
        ...
        try:
            ...
            from reames_cli.plugins import _ensure_plugins_discovered
    ...
    if subscription_features.video_gen.managed_by_nous:
        ...
        try:
            ...
            from reames_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
    ...
    if disabled_tools:
        ...
        from reames_constants import display_hermes_home as _dhh
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    from reames_cli.main import select_provider_and_model
    try:
...
def _setup_telegram_auto_result():
    ...
    try:
        from reames_cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None
...
def _write_slack_manifest_and_instruct():
    ...
    """
    try:
        from reames_cli.slack_cli import _build_full_manifest
...
def _setup_qqbot():
    ...
    from reames_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def setup_gateway(config: dict):
    ...
    from reames_cli.gateway import _all_platforms, _platform_status, _configure_platform
    ...
    if any_messaging:
        ...
        if service_running:
            if supports_systemd and _system_scope_wizard_would_need_root():
                ...
                try:
                    if supports_systemd:
                        ...
                        from reames_cli import gateway_windows
            ...
            from reames_constants import is_container
            if is_container():
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``reames_cli.auth`` — lists every supported
    ...
    """
    try:
        from reames_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from reames_cli.config
    ...
    if section_key == "model":
        ...
        from reames_cli.gateway import _all_platforms, _platform_status
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    try:
        from reames_cli.main import _model_flow_nous
...
def run_setup_wizard(args):
    ...
    """
    from reames_cli.config import is_managed, managed_error
    if is_managed():

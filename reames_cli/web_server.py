...
Usage:
    python -m reames_cli.main web          # Start on http://127.0.0.1:9119
    python -m reames_cli.main web --port 8080
"""
...
#
# This list is defined in ``reames_cli.dashboard_auth.public_paths`` so the
...
# ---------------------------------------------------------------------------
from reames_cli.dashboard_auth.public_paths import (
...
async def _dashboard_auth_gate(request: Request, call_next):
    from reames_cli.dashboard_auth.middleware import gated_auth_middleware
...
async def get_status():
    ...
    try:
        from reames_cli.dashboard_auth import list_providers as _list_providers
...
async def get_portal_status():
    ...
    auth: Dict[str, Any] = {}
    try:
        from reames_cli.auth import get_nous_auth_status
...
async def run_debug_share_endpoint(body: DebugShareRequest | None = None):
    ...
    *synchronous*: the whole point of ``debug share`` is the set of shareable
    ...
    """
    from reames_cli.debug import build_debug_share
...
def _spawn_hermes_action(subcommand: List[str], name: str) -> subprocess.Popen:
    ...
    Uses the running interpreter's ``reames_cli.main`` module so the action
    ...
    cmd = [sys.executable, "-m", "reames_cli.main", *subcommand]
...
def get_recommended_default_model(provider: str = ""):
    ...
    if slug == "nous":
        try:
            ...
            from reames_cli.auth import get_provider_auth_state
    ...
    try:
        ...
        if scope == "main":
            ...
            if provider.strip().lower() == "nous":
                try:
                    from reames_cli.nous_subscription import apply_nous_managed_defaults
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from reames_cli import auth as hauth
        if provider_id == "nous":
...
async def disconnect_oauth_provider(provider_id: str, request: Request):
    ...
    if provider_id in {"anthropic", "claude-code"}:
        ...
        try:
            from reames_cli.auth import clear_provider_auth
...
def _nous_poller(session_id: str) -> None:
    ...
    try:
        ...
        from reames_cli.auth import persist_nous_credentials
...
def _codex_full_login_worker(session_id: str) -> None:
    ...
    try:
        ...
        from reames_cli.auth import _save_codex_tokens
...
async def get_logs(
    ...
    from reames_cli.logs import _read_tail, LOG_FILES
    ...
    try:
        from reames_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
#
# Wraps the same config data layer the CLI uses (reames_cli.mcp_config), so
...
async def add_mcp_server(body: MCPServerCreate):
    from reames_cli.mcp_config import _get_mcp_servers, _save_mcp_server
...
async def remove_mcp_server(name: str):
    from reames_cli.mcp_config import _remove_mcp_server
...
async def test_mcp_server(name: str):
    ...
    from reames_cli.mcp_config import _get_mcp_servers, _probe_single_server
...
async def list_mcp_catalog():
    ...
    """
    try:
        from reames_cli import mcp_catalog
    except Exception as exc:
...
#
# Wraps the same JSON store the CLI uses (reames_cli.webhook); the webhook
...
async def list_webhooks():
    import reames_cli.webhook as wh
...
async def list_credential_pool():
    ...
    from reames_cli.auth import read_credential_pool
...
async def list_hooks():
    ...
    try:
        from reames_cli.plugins import VALID_HOOKS
...
async def preview_skill_hub(identifier: str = ""):
    ...
    def _run():
        from reames_cli.skills_hub import _resolve_source_meta_and_bundle
        from tools.skills_hub import create_source_router
...
def _write_profile_model(profile_dir: Path, provider: str, model: str) -> None:
    ...
    """
    from reames_constants import set_hermes_home_override, reset_hermes_home_override
...
async def describe_profile_auto_endpoint(name: str, body: ProfileDescribeAuto):
    ...
    _resolve_profile_dir(name)
    try:
        from reames_cli import profile_describer
...
async def get_skills():
    ...
    from reames_cli.skills_config import get_disabled_skills
    config = load_config()
...
async def toggle_skill(body: SkillToggle):
    from reames_cli.skills_config import get_disabled_skills, save_disabled_skills
    config = load_config()
    disabled = get_disabled_skills(config)
    if body.enabled:
...
try:
    from reames_cli.pty_bridge import PtyBridge, PtyUnavailableError
...
def _ws_auth_reason(ws: "WebSocket") -> tuple[Optional[str], str]:
    ...
    * ``?internal=<process-credential>`` — the process-lifetime internal
    ...
    if auth_required:
        ...
        from reames_cli.dashboard_auth.audit import AuditEvent, audit_log
        from reames_cli.dashboard_auth.ws_tickets import (
            TicketInvalid,
...
def _resolve_chat_argv(
    ...
    matching what ``reames_cli.main._launch_tui`` does for the CLI path.
    ...
    """
    from reames_cli.main import PROJECT_ROOT, _make_tui_argv
...
def _build_gateway_ws_url() -> Optional[str]:
    ...
    if getattr(app.state, "auth_required", False):
        from reames_cli.dashboard_auth.ws_tickets import internal_ws_credential
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Thin re-export of :func:`reames_cli.dashboard_auth.prefix.normalise_prefix`
    ...
    """
    from reames_cli.dashboard_auth.prefix import normalise_prefix
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as reames_cli.plugins):
    ...
    from reames_cli.plugins import get_bundled_plugins_dir
    ...
    if env_var_enabled("HERMES_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((get_hermes_home() / "plugins", "project"))
...
def _merged_plugins_hub() -> Dict[str, Any]:
    ...
    from reames_cli.plugins_cmd import (
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from reames_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from reames_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from reames_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from reames_cli.plugins_cmd import dashboard_remove_user_plugin
...
from reames_cli.dashboard_auth.routes import router as _dashboard_auth_router  # noqa: E402
...
def start_server(
    ...
    if app.state.auth_required:
        ...
        from reames_cli.dashboard_auth import list_providers

...
def find_profile_gateway_processes(
    ...
    try:
        from gateway.status import get_running_pid
        from reames_cli.profiles import list_profiles
    except Exception:
        return processes
...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    args = [get_python_path(), "-m", "reames_cli.main"]
    if profile != "default":
...
def launch_detached_profile_gateway_restart(profile: str, old_pid: int) -> bool:
    ...
    from reames_cli._subprocess_compat import (
...
def _gateway_list() -> None:
    ...
    """
    try:
        from reames_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
from reames_constants import is_container, is_termux, is_wsl
...
def _hermes_home_for_target_user(target_home_dir: str) -> str:
    ...
    target_default = Path(target_home_dir) / ".reames"
...
def _stable_service_working_dir() -> str:
    ...
    uses an absolute python interpreter and ``-m reames_cli.main``, so module
...
User={username}
Group={group_name}
ExecStart={python_path} -m reames_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
...
def _gateway_run_command() -> list[str]:
    """Build the `python -m reames_cli.main [--profile X] gateway run --replace` argv.
    ...
    """
    cmd = [get_python_path(), "-m", "reames_cli.main"]
...
def _setup_standard_platform(platform: dict):
    ...
    if platform.get("key") == "telegram":
        ...
        if choice.strip() == "1":
            try:
                from reames_cli.telegram_managed_bot import (
...
def _setup_whatsapp():
    ...
    from reames_cli.main import cmd_whatsapp
    import argparse
...
def _setup_dingtalk():
    ...
    if method == 0:
        ...
        try:
            from reames_cli.dingtalk_auth import dingtalk_qr_auth
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``reames_cli.setup`` (which
    imports from this module for the remaining bespoke flows).
    """
    from reames_cli import setup as _s
...
def _dispatch_via_service_manager_if_s6(
    ...
    :mod:`reames_cli.service_manager` are caught and surfaced as

...
def uninstall_gateway_service():
    ...
    try:
        from reames_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from reames_cli.gateway import get_launchd_plist_path
...
def _uninstall_profile(profile) -> None:
    ...
    #    Use `python -m reames_cli.main` so we don't depend on a `hermes`
    #    wrapper that may be half-removed mid-uninstall.
    hermes_invocation = [_sys.executable, "-m", "reames_cli.main", "--profile", name]
...
def run_gui_uninstall(args):
    ...
    """
    from reames_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
...
def _perform_uninstall(
    *,
    ...
    try:
        from reames_cli.gui_uninstall import uninstall_gui
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m reames_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import reames_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``hermes_constants`` + ``reames_cli.colors``
    (and lazily ``reames_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m reames_cli.uninstall")
    parser.add_argument(
        "--mode",

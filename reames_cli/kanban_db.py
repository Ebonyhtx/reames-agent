...
def _canonical_assignee(assignee: Optional[str]) -> Optional[str]:
    ...
    if assignee is None:
        return None
    from reames_cli.profiles import normalize_profile_name
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    if not rows:
        return False
    try:
        from reames_cli.profiles import profile_exists  # local import: avoids cycle
    except Exception:
...
def dispatch_once(
    ...
    *,
    ...
    if _default_assignee:
        try:
            from reames_cli.profiles import profile_exists as _pe
...
def _module_hermes_argv() -> list[str]:
    ...
    # ``reames_cli.main`` is the console-script target declared in
...
def _resolve_hermes_argv() -> list[str]:
    ...
    3. ``sys.executable -m reames_cli.main`` — fallback for setups where
...
def list_profiles_on_disk() -> list[str]:
    ...
    ``reames_cli.profiles`` (which pulls in a large chunk of the CLI startup
    path).
    """
    try:
        from reames_constants import get_default_hermes_root

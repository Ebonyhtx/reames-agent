...
* We write two files: a shared ``gateway.cmd`` wrapper script (cwd + env + the
  actual ``python -m reames_cli.main gateway run --replace`` invocation) and
...
def _current_profile_cli_args() -> list[str]:
    ...
    from reames_cli.gateway import _profile_arg
...
def _launch_elevated_gateway_command(command: str, extra_args: list[str] | None = None) -> bool:
    ...
    """
    _assert_windows()
    args = ["-m", "reames_cli.main", *_current_profile_cli_args(), "gateway", command]
    if extra_args:
...
def get_task_name() -> str:
    ...
    from reames_cli.gateway import _profile_suffix
...
def _build_gateway_cmd_script(
    ...
    The script:
      ...
      - invokes ``pythonw -m reames_cli.main [--profile X] gateway run``
    ...
    prog_args = [pythonw_path, "-m", "reames_cli.main"]
    if profile_arg:
...
def _build_gateway_argv() -> tuple[list[str], str, dict[str, str]]:
    ...
    argv = [python_exe, "-m", "reames_cli.main"]
    if profile_arg:
...
def _spawn_detached(script_path: Path | None = None) -> int:
    ...
    We spawn ``pythonw.exe -m reames_cli.main gateway run``
...
def _prompt_install_choices(
    ...
    from reames_cli.setup import prompt_yes_no
...
def _install_startup_fallback(script_path: Path, start_now: bool, detail: str) -> None:
    ...
    from reames_cli.gateway import find_gateway_pids, _profile_arg
...
def stop() -> None:
    ...
    """
    _assert_windows()
    from reames_cli.gateway import kill_gateway_processes, _get_restart_drain_timeout
    from gateway.status import get_running_pid

...
# that run *before* reames_cli.config is importable. Mirrors the explicit
...
def _print_fast_version_info() -> None:
    from reames_cli import __release_date__, __version__
...
from reames_cli.subcommands._shared import add_accept_hooks_flag as _add_accept_hooks_flag
from reames_cli.subcommands.cron import build_cron_parser
from reames_cli.subcommands.gateway import build_gateway_parser
from reames_cli.subcommands.profile import build_profile_parser
...
from reames_cli.subcommands.memory import build_memory_parser
from reames_cli.subcommands.acp import build_acp_parser
from reames_cli.subcommands.tools import build_tools_parser
from reames_cli.subcommands.insights import build_insights_parser
from reames_cli.subcommands.skills import build_skills_parser
from reames_cli.subcommands.pairing import build_pairing_parser
from reames_cli.subcommands.plugins import build_plugins_parser
from reames_cli.subcommands.mcp import build_mcp_parser
from reames_cli.subcommands.claw import build_claw_parser
...
def _apply_profile_override() -> None:
    ...
    # Mirrors reames_cli.profiles._PROFILE_ID_RE so we never call
    ...
    if profile_name is None:
        try:
            from reames_constants import get_default_hermes_root
    ...
    if profile_name is not None:
        try:
            from reames_cli.profiles import resolve_profile_env
...
try:
    from reames_logging import setup_logging as _setup_logging
...
if _FORCE_IPV4_EARLY:
    try:
        from reames_constants import apply_ipv4_preference as _apply_ipv4
...
def _has_any_provider_configured() -> bool:
    ...
    from reames_cli.config import get_env_path, get_hermes_home, load_config
    from reames_cli.auth import get_auth_status

    # Determine whether Reames itself has been explicitly configured (model
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    ...
    def _node_bin(bin: str) -> str:
        ...
        if not path and bin == "node":
            try:
                from reames_cli.dep_ensure import ensure_dependency
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from reames_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    # sys.executable can be a directory on some embedded Python builds
    # (e.g. LibreOffice on Windows), which makes TUI's gateway spawn fail
    # with ENOENT.  Walk up to find the actual python.exe.
    _py = sys.executable
    if os.path.isdir(_py):
        _candidates = [
            os.path.join(_py, "python.exe"),
            os.path.join(os.path.dirname(_py), "python.exe"),
        ]
        _py = next((c for c in _candidates if os.path.isfile(c)), "python")
    env.setdefault("HERMES_PYTHON", _py)
    ...
    if code == 42:
        from reames_cli.relaunch import relaunch
...
def _pin_kanban_board_env() -> None:
    ...
    if os.environ.get("HERMES_KANBAN_BOARD"):
        return
    try:
        from reames_cli.kanban_db import get_current_board
...
def cmd_chat(args):
    ...
    try:
        from reames_cli.xai_retirement import (
            MIGRATION_GUIDE_URL,
            RETIREMENT_DATE,
            find_retired_xai_refs,
            format_issue,
        )
        from reames_cli.config import load_config as _load_config_for_xai_check
...
def cmd_gateway(args):
    ...
    from reames_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from reames_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_setup(args):
    ...
    from reames_cli.setup import run_setup_wizard
...
def cmd_postinstall(args):
    ...
    from reames_cli.config import stamp_install_method
    from reames_cli.dep_ensure import ensure_dependency
...
def select_provider_and_model(args=None):
    ...
    persistence.
    """
    from reames_cli.auth import (
        resolve_provider,
        AuthError,
        format_auth_error,
    )
    from reames_cli.config import (
        get_compatible_custom_providers,
        load_config,
        get_env_value,
    )
    from reames_cli.providers import resolve_provider_full
...
def _clear_stale_openai_base_url():
    ...
    """
    from reames_cli.config import get_env_value, save_env_value, load_config
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    :meth:`reames_cli.plugins.PluginContext.register_auxiliary_task`) appear
...
def _aux_select_for_task(task: str) -> None:
    ...
    """
    from reames_cli.config import load_config
    from reames_cli.model_switch import list_authenticated_providers
...
def _aux_flow_provider_model(
    ...
    from reames_cli.auth import _prompt_model_selection
    from reames_cli.models import get_pricing_for_provider
...
def _aux_flow_custom_endpoint(task: str, task_cfg: dict) -> None:
    ...
    from reames_cli.secret_prompt import masked_secret_prompt
...
def _prompt_provider_choice(choices, *, default=0):
    ...
    """
    try:
        from reames_cli.setup import _curses_prompt_choice
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from reames_constants import OPENROUTER_BASE_URL
    from reames_cli.auth import (
        ProviderConfig,
    ...
    # bad paste short of re-running `reames setup` from scratch. OpenRouter
    ...
    from reames_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from reames_cli.nous_subscription import prompt_enable_tool_gateway
    ...
    if free_tier:
        try:
            from reames_cli.nous_account import (
    ...
    if free_tier and not model_ids:
        ...
        if unavailable_models:
            from reames_cli.auth import DEFAULT_NOUS_PORTAL_URL
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    from reames_cli.codex_models import get_codex_model_ids
...
def _model_flow_xai_oauth(_config, current_model="", *, args=None):
    ...
    from reames_cli.models import _PROVIDER_MODELS
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from reames_cli.models import fetch_api_models
...
def _model_flow_custom(config):
    ...
    """
    from reames_cli.auth import _save_model_choice, deactivate_provider
    from reames_cli.config import get_env_value, load_config, save_config
    from reames_cli.secret_prompt import masked_secret_prompt
    ...
    from reames_cli.models import probe_api_models
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    """
    from reames_cli.runtime_provider import _detect_api_mode_for_url
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    """
    from reames_cli.auth import _save_model_choice, deactivate_provider  # noqa: F401
    from reames_cli.config import (
        get_env_value,
        save_env_value,
        load_config,
    ...
    from reames_cli import azure_detect
...
def _remove_custom_provider(config):
    ...
    try:
        from reames_cli.curses_ui import curses_radiolist
...
# downstream call sites read `reames_cli.main._PROVIDER_MODELS` directly,
...
def _model_flow_copilot(config, current_model=""):
    ...
    from reames_cli.config import save_env_value, load_config, save_config
    from reames_cli.models import (
        _PROVIDER_MODELS,
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from reames_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from reames_cli.copilot_auth import validate_copilot_token
        ...
        if source in {"GITHUB_TOKEN", "GH_TOKEN"}:
            from reames_cli.env_loader import format_secret_source_suffix
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    """Shared API-key entry point for ``reames setup`` / ``hermes model``.
    ...
    """
    from reames_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    ...
    if choice.startswith("c"):
        ...
        print(
            f"  API key cleared.  Re-run `reames setup` to configure {pconfig.name} again."
        )
        return "", True
...
def _model_flow_stepfun(config, current_model=""):
    ...
    from reames_cli.models import _PROVIDER_MODELS, fetch_api_models
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    if provider_id == "lmstudio":
        from reames_cli.auth import AuthError
        from reames_cli.models import fetch_lmstudio_models
        ...
        from reames_cli.models import fetch_ollama_cloud_models
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from reames_constants import display_hermes_home as _dhh_fn
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from reames_cli.auth import get_anthropic_key
...
def cmd_login(args):
    ...
    from reames_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from reames_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from reames_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from reames_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from reames_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from reames_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    if sub == "manifest":
        from reames_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from reames_cli.kanban import kanban_command
...
def cmd_hooks(args):
    ...
    from reames_cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from reames_cli.doctor import run_doctor
...
def cmd_security(args):
    ...
    if sub in ("audit", None):
        from reames_cli.security_audit import cmd_security_audit
...
def cmd_dump(args):
    ...
    from reames_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from reames_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from reames_cli.config import config_command
...
def cmd_backup(args):
    ...
    if getattr(args, "quick", False):
        from reames_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from reames_cli.backup import run_backup
...
def cmd_import(args):
    ...
    from reames_cli.backup import run_import
...
def _print_version_info(*, check_updates: bool = True) -> None:
    ...
    try:
        from reames_cli.banner import check_for_updates
...
def cmd_uninstall(args):
    ...
    if getattr(args, "gui_summary", False):
        from reames_cli.gui_uninstall import gui_install_summary
    ...
    if getattr(args, "gui", False):
        ...
        from reames_cli.uninstall import run_gui_uninstall
    ...
    from reames_cli.uninstall import run_uninstall
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    # build path usable on a stock `py -m reames_cli.main web` invocation.
...
def cmd_gui(args: argparse.Namespace):
    ...
    try:
        from reames_logging import setup_logging as _setup_logging_gui
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    ...
    try:
        if sys.platform == "win32":
            ...
            # with `reames_cli.gateway._scan_gateway_pids` and avoids the
...
def _update_via_zip(args):
    ...
    from reames_cli.managed_uv import ensure_uv, update_managed_uv
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from reames_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    """
    from reames_cli.managed_uv import resolve_uv
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    try:
        ...
        # reames_cli.config.get_hermes_home to simulate setup failure.
        from reames_cli.config import get_hermes_home as _get_hermes_home
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    ...
    dropping the flag.
    """
    from reames_cli.config import detect_install_method
    ...
    if method == "docker":
        ...
        from reames_cli.config import format_docker_update_message
        print(format_docker_update_message())
        sys.exit(1)
    if method == "pip":
        from reames_cli.config import recommended_update_command
        from reames_cli.banner import check_via_pypi
...
def _run_pre_update_backup(args) -> None:
    ...
    try:
        from reames_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    try:
        from reames_constants import get_hermes_home, display_hermes_home
...
def _cmd_update_pip(args):
    ...
    from reames_cli.config import is_uv_tool_install
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        try:
            from reames_cli.backup import create_quick_snapshot
        ...
        if (desktop_dir / "package.json").exists() and shutil.which("npm") and has_desktop_app:
            ...
            _desktop_build_cmd = [sys.executable, "-m", "reames_cli.main", "desktop", "--build-only"]
        ...
        try:
            import importlib
            import reames_constants as _hc
        ...
        try:
            from reames_cli.profiles import (
                list_profiles,
                seed_profile_skills,
            )
        ...
        try:
            from reames_cli.backup import restore_cron_jobs_if_emptied
        ...
        try:
            if sys.platform == "darwin" and shutil.which("cua-driver"):
                from reames_cli.tools_config import install_cua_driver
        ...
        try:
            from reames_cli.gateway import (
                is_macos,
...
def cmd_profile(args):
    ...
    if action == "list":
        ...
        from reames_cli import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from reames_constants import get_hermes_home as _hh
        ...
        from reames_cli import profile_describer as _pd
        ...
        from reames_cli.profiles import profile_exists
        ...
        from reames_cli.profiles import rename_profile
        ...
        from reames_cli.profiles import export_profile
        ...
        from reames_cli.profiles import import_profile
        ...
        import tempfile
        from reames_cli.profile_distribution import (
            plan_install,
        ...
        from reames_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        from reames_cli.profile_distribution import describe_distribution, DistributionError
...
def _render_distribution_plan(plan) -> None:
    ...
    from reames_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
...
def cmd_dashboard(args):
    ...
    from reames_cli.web_server import start_server
...
def cmd_dashboard_register(args):
    ...
    from reames_cli.dashboard_register import cmd_dashboard_register as _impl
...
def cmd_completion(args, parser=None):
    ...
    from reames_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_prompt_size(args):
    ...
    from reames_cli.prompt_size import cmd_prompt_size as _impl
...
def cmd_logs(args):
    ...
    from reames_cli.logs import tail_log, list_logs
...
def _prepare_agent_startup(args) -> None:
    ...
    if _is_tui_chat_launch(args):
        ...
        try:
            from reames_cli.mcp_startup import start_background_mcp_discovery
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    from reames_cli._parser import build_top_level_parser
    ...
    if getattr(args, "oneshot", None):
        ...
        from reames_cli.oneshot import run_oneshot
...
def cmd_memory(args):
    ...
    if sub == "off":
        ...
        from reames_cli.memory_setup import memory_command
...
def cmd_tools(args):
    ...
    if action in {"list", "disable", "enable"}:
        from reames_cli.tools_config import tools_disable_enable_command

        tools_disable_enable_command(args)
    elif action == "post-setup":
        from reames_cli.tools_config import run_post_setup_command

        sys.exit(run_post_setup_command(args))
    else:
        _require_tty("tools")
        from reames_cli.tools_config import tools_command
...
def cmd_skills(args):
    ...
    if getattr(args, "skills_action", None) == "config":
        ...
        from reames_cli.skills_config import skills_command as skills_config_command

        skills_config_command(args)
    else:
        from reames_cli.skills_hub import skills_command

        skills_command(args)


def cmd_pairing(args):
    from reames_cli.pairing import pairing_command

    pairing_command(args)


def cmd_plugins(args):
    from reames_cli.plugins_cmd import plugins_command
...
def cmd_claw(args):
    from reames_cli.claw import claw_command
...
def main():
    ...
    # =========================================================================
    from reames_cli.fallback_cmd import cmd_fallback
    ...
    from reames_cli import secrets_cli as _secrets_cli
    ...
    # migrate command
    # =========================================================================
    from reames_cli.migrate import cmd_migrate, cmd_migrate_xai
    ...
    # =========================================================================
    from reames_cli.send_cmd import register_send_subparser
    ...
    # =========================================================================
    from reames_cli.portal_cli import add_parser as _add_portal_parser
    _add_portal_parser(subparsers)

    # =========================================================================
    # kanban command — multi-profile collaboration board
    # =========================================================================
    from reames_cli.kanban import build_parser as _build_kanban_parser
    ...
    from reames_cli.checkpoints import register_cli as _register_checkpoints_cli
    ...
    from reames_cli.bundles import register_cli as _bundles_register, bundles_command
    ...
    if _plugin_cli_discovery_needed():
        try:
            ...
            from reames_cli.plugins import discover_plugins, get_plugin_manager
    ...
    try:
        from reames_cli.curator import register_cli as _register_curator_cli
    ...
    from reames_cli.config import get_container_exec_info

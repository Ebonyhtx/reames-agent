...
def _enter_existing_install_patches(stack, **extra):
    ...
    for target, kwargs in [
        ("reames_cli.setup.ensure_hermes_home", {}),
        ("reames_cli.setup.is_interactive_stdin", {"return_value": True}),
        ("reames_cli.config.is_managed", {"return_value": False}),
        ("reames_cli.setup.load_config", {"return_value": {}}),
        ("reames_cli.setup.save_config", {}),
        ("reames_cli.setup.get_env_value", {"return_value": None}),
        ("reames_cli.auth.get_active_provider", {"return_value": "openrouter"}),
        ("reames_cli.setup._print_setup_summary", {}),
        ("reames_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
...
def _enter_fresh_install_patches(stack, **extra):
    for target, kwargs in [
        ...
        ("reames_cli.setup.load_config", {"return_value": {}}),
        ("reames_cli.setup.save_config", {}),
        ("reames_cli.auth.get_active_provider", {"return_value": None}),
        ("reames_cli.setup.get_env_value", {"return_value": None}),
        ("reames_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
        stack.enter_context(patch(target, **kwargs))
    ...
    def test_bare_setup_runs_full_reconfigure_without_menu(self, existing_install):
        ...
        with ExitStack() as stack:
            m = _enter_existing_install_patches(
                stack,
                prompt_choice="reames_cli.setup.prompt_choice",
                quick="reames_cli.setup._run_quick_setup",
                model="reames_cli.setup.setup_model_provider",
                terminal="reames_cli.setup.setup_terminal_backend",
                agent="reames_cli.setup.setup_agent_settings",
                gateway="reames_cli.setup.setup_gateway",
                tools="reames_cli.setup.setup_tools",
            )
    ...
    def test_bare_setup_runs_first_time_flow(self, fresh_install):
        ...
        with ExitStack() as stack:
            m = _enter_fresh_install_patches(
                stack,
                prompt=("reames_cli.setup.prompt_choice", {"return_value": 0}),
                first="reames_cli.setup._run_first_time_quick_setup",
            )
            from reames_cli.setup import run_setup_wizard
            run_setup_wizard(args)
...
class TestArgparse:
    ...
    def test_reconfigure_flag_reaches_cmd_setup(self, monkeypatch):
        ...
        captured = {}
        monkeypatch.setattr(
            "reames_cli.setup.run_setup_wizard",

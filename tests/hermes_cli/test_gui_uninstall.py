"""Tests for reames_cli.gui_uninstall — GUI-only uninstall + install discovery.
...
import reames_cli.gui_uninstall as gu
...
def test_run_uninstall_yes_keep_data_is_non_interactive(tmp_path, monkeypatch):
    ...
    """
    import reames_cli.uninstall as uninstall
    ...
    from reames_cli import gui_uninstall as gu_mod
...
def test_uninstall_module_main_gui_mode(tmp_path, monkeypatch):
    """`python -m reames_cli.uninstall --mode gui` runs the GUI-only path.

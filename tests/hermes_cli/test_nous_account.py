...
def test_valid_jwt_with_paid_access_true(monkeypatch):
    ...
    monkeypatch.setattr("reames_cli.auth.get_provider_auth_state", lambda provider: _state(token))
...
def test_expired_jwt_falls_back_to_fresh_account(monkeypatch):
    ...
    monkeypatch.setattr("reames_cli.auth.get_provider_auth_state", lambda provider: _state(token))
    monkeypatch.setattr("reames_cli.auth.resolve_nous_access_token", lambda: "fresh-token")
    monkeypatch.setattr("reames_cli.nous_account._fetch_nous_account_info", lambda *a, **kw: payload)
...
def test_no_oauth_token_reports_inference_key_present(monkeypatch):
    monkeypatch.setattr("reames_cli.auth.get_provider_auth_state", lambda provider: {})

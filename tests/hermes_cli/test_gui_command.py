...
from reames_cli import main as cli_main
...
def test_gui_installs_packages_and_launches_desktop_app(tmp_path, monkeypatch):
    ...
    with patch("reames_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("reames_cli.main._run_npm_install_deterministic", return_value=install_ok) as mock_install, \
         patch("reames_cli.main._desktop_build_needed", return_value=True), \
         patch("reames_cli.main._write_desktop_build_stamp"), \
         patch("reames_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("reames_cli.main.subprocess.run", side_effect=[pack_ok, launch_ok]) as mock_run, \
...
def test_gui_forwards_desktop_environment_overrides(tmp_path, monkeypatch):
    ...
    with patch("reames_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("reames_cli.main._run_npm_install_deterministic", return_value=ok), \
         patch("reames_cli.main._desktop_build_needed", return_value=True), \
         patch("reames_cli.main._write_desktop_build_stamp"), \
         patch("reames_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("reames_cli.main.subprocess.run", side_effect=[ok, ok]) as mock_run, \
...
def test_gui_exits_when_npm_missing(tmp_path, monkeypatch, capsys):
    ...
    with patch("reames_cli.main.shutil.which", return_value=None), \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns())
...
def test_gui_skip_build_launches_existing_packaged_app_without_npm(tmp_path, monkeypatch):
    ...
    with patch("reames_cli.main.shutil.which", return_value=None), \
         patch("reames_cli.main._run_npm_install_deterministic") as mock_install, \
         patch("reames_cli.main.subprocess.run", return_value=launch_ok) as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns(skip_build=True))
...
def test_gui_linux_configures_sandbox_before_launch(tmp_path, monkeypatch):
    ...
    with patch("reames_cli.main.shutil.which", return_value="/usr/bin/sudo"), \
         patch("reames_cli.main.subprocess.run", return_value=ok) as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns(skip_build=True))
...
def test_gui_source_mode_uses_renderer_build_and_electron(tmp_path, monkeypatch):
    ...
    with patch("reames_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("reames_cli.main._run_npm_install_deterministic", return_value=install_ok), \
         patch("reames_cli.main._desktop_build_needed", return_value=True), \
         patch("reames_cli.main._write_desktop_build_stamp"), \
         patch("reames_cli.main.subprocess.run", side_effect=[build_ok, launch_ok]) as mock_run, \
         pytest.raises(SystemExit) as exc:
...
def test_desktop_build_stamp_skips_build_when_up_to_date(tmp_path, monkeypatch):
    ...
    with patch("reames_cli.main._desktop_build_needed", return_value=False), \
         patch("reames_cli.main._run_npm_install_deterministic") as mock_install, \
         patch("reames_cli.main.subprocess.run", return_value=launch_ok) as mock_run, \
         patch("reames_cli.main._desktop_macos_relaunchable_fixup"), \
         pytest.raises(SystemExit) as exc:
...
def test_desktop_force_build_overrides_stamp(tmp_path, monkeypatch):
    ...
    with patch("reames_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("reames_cli.main._run_npm_install_deterministic", return_value=install_ok) as mock_install, \
         patch("reames_cli.main._desktop_build_needed", return_value=False), \
         patch("reames_cli.main._write_desktop_build_stamp") as mock_stamp, \
         patch("reames_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("reames_cli.main.subprocess.run", side_effect=[pack_ok, launch_ok]) as mock_run, \
         pytest.raises(SystemExit) as exc:
...
def test_gui_retries_pack_once_after_purging_build_cache(tmp_path, monkeypatch):
    ...
    with patch("reames_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("reames_cli.main._run_npm_install_deterministic", return_value=install_ok), \
         patch("reames_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("reames_cli.main._desktop_linux_sandbox_fixup", return_value=True), \
         patch("reames_cli.main._write_desktop_build_stamp"), \
         patch("reames_cli.main._purge_electron_build_cache", return_value=[Path("/c/electron.zip")]) as mock_purge, \
         patch("reames_cli.main.subprocess.run", side_effect=[pack_fail, pack_ok, launch_ok]) as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns())
...
def test_gui_does_not_retry_when_purge_finds_nothing(tmp_path, monkeypatch, capsys):
    ...
    with patch("reames_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("reames_cli.main._run_npm_install_deterministic", return_value=install_ok), \
         patch("reames_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("reames_cli.main._purge_electron_build_cache", return_value=[]) as mock_purge, \
         patch("reames_cli.main.subprocess.run", side_effect=[pack_fail]) as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns())

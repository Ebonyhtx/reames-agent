...
class TestLoadConfigParseFailure:
    ...
    def test_logs_and_warns_on_parse_failure(self, tmp_path, caplog, capsys):
        ...
        from reames_cli import config as cfg_mod
        ...
        with patch.dict(os.environ, {"HERMES_HOME": str(tmp_path)}):
            ...
            import logging
            with caplog.at_level(logging.WARNING, logger="reames_cli.config"):
                config = load_config()
...
class TestOptionalEnvVarsRegistry:
    ...
    def test_tavily_in_env_vars_by_version(self):
        ...
        from reames_cli.config import ENV_VARS_BY_VERSION
        all_vars = []

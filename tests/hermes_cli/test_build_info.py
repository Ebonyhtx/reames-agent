"""Tests for reames_cli.build_info — baked-in build SHA resolution.

The build SHA is written by the Dockerfile's ``HERMES_GIT_SHA`` build-arg
into ``<project_root>/.reames_build_sha``.  These tests cover the read-side
...
def test_get_build_sha_returns_none_when_file_absent(tmp_path):
    ...
    from reames_cli import build_info

    missing = tmp_path / ".reames_build_sha"  # never created
...
def test_get_build_sha_reads_baked_file(tmp_path):
    ...
    sha_file = tmp_path / ".reames_build_sha"

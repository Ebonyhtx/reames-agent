"""Tests for reames_cli.tools_config platform tool persistence."""
...
def test_get_platform_tools_x_search_auto_enabled_when_xai_oauth_present(monkeypatch):
    ...
    monkeypatch.setattr(
        "reames_cli.tools_config._xai_credentials_present", lambda: True
    )
...
def test_get_platform_tools_x_search_off_when_no_xai_credentials(monkeypatch):
    ...
    monkeypatch.delenv("XAI_API_KEY", raising=False)
    monkeypatch.setattr(
        "reames_cli.tools_config._xai_credentials_present", lambda: False
    )
...
def test_apply_toolset_change_from_default_does_not_enable_default_off_toolsets():
    ...
    with patch("reames_cli.tools_config.save_config"):
...
def test_visible_providers_force_fresh_shows_nous_subscription_after_upgrade(monkeypatch):
    ...
    monkeypatch.setattr(
        "reames_cli.tools_config.get_nous_subscription_features",
...
def test_local_browser_provider_is_saved_explicitly(monkeypatch):
    ...
    monkeypatch.setattr("reames_cli.tools_config._run_post_setup", lambda key: None)
...
def test_fresh_install_browser_default_is_free_local_not_paid_nous():
    ...
    """
    from reames_cli.tools_config import _detect_active_provider_index
...
def test_reconfigure_lists_enabled_web_without_existing_provider_config(monkeypatch):
    ...
    monkeypatch.setattr(
        "reames_cli.tools_config._toolset_has_keys",
    ...
    monkeypatch.setattr("reames_cli.tools_config._prompt_choice", fake_prompt_choice)
    monkeypatch.setattr(
        "reames_cli.tools_config._configure_tool_category_for_reconfig",
        lambda ts_key, cat, config, **kwargs: configured.append(ts_key),
    )
    monkeypatch.setattr("reames_cli.tools_config.save_config", lambda config: None)
...
def test_first_install_nous_auto_configures_managed_defaults(monkeypatch):
    monkeypatch.setattr("reames_cli.nous_subscription.managed_nous_tools_enabled", lambda: True)
    config = {
        "model": {"provider": "nous"},
        "platform_toolsets": {"cli": []},
    }
    ...
    monkeypatch.setattr(
        "reames_cli.tools_config._prompt_toolset_checklist",
    ...
    monkeypatch.setattr(
        "reames_cli.tools_config._get_enabled_platforms",
        lambda: ["cli"],
    )
    monkeypatch.setattr(
        "reames_cli.nous_subscription.get_nous_portal_account_info",
    ...
    configured = []
    monkeypatch.setattr(
        "reames_cli.tools_config._configure_toolset",
...
class TestPlatformToolsetConsistency:
    ...
    def test_all_platforms_have_toolset_definitions(self):
        ...
        from reames_cli.tools_config import PLATFORMS
    ...
    def test_skills_config_covers_tools_config_platforms(self):
        ...
        from reames_cli.tools_config import PLATFORMS as TOOLS_PLATFORMS
        from reames_cli.skills_config import PLATFORMS as SKILLS_PLATFORMS
...
class TestImagegenBackendRegistry:
    ...
    def test_fal_backend_registered(self):
        from reames_cli.tools_config import IMAGEGEN_BACKENDS
    ...
    def test_image_gen_providers_tagged_with_fal_backend(self):
        ...
        from reames_cli.tools_config import TOOL_CATEGORIES
...
class TestImagegenModelPicker:
    ...
    def test_picker_writes_chosen_model_to_config(self):
        from reames_cli.tools_config import _configure_imagegen_model
        config = {}
        # Force _prompt_choice to pick index 1 (second-in-ordered-list).
        with patch("reames_cli.tools_config._prompt_choice", return_value=1):
    ...
    def test_picker_with_gpt_image_does_not_prompt_quality(self):
        ...
        config = {}
        with patch("reames_cli.tools_config._prompt_choice", side_effect=fake_prompt):
            _configure_imagegen_model("fal", config)
    ...
    def test_picker_repairs_corrupt_config_section(self):
        ...
        with patch("reames_cli.tools_config._prompt_choice", return_value=0):
            _configure_imagegen_model("fal", config)
...
def test_get_platform_tools_recovers_non_configurable_toolsets_from_composite():
    ...
    with mock_patch("reames_cli.tools_config.PLATFORMS", {**PLATFORMS, **test_platforms}):
...
def test_discord_toolsets_not_available_on_other_platforms():
    ...
    from reames_cli.tools_config import _toolset_allowed_for_platform
...
def test_save_platform_tools_strips_restricted_toolsets():
    ...
    from reames_cli.tools_config import _save_platform_tools
    config = {}
...
def test_get_effective_configurable_toolsets_dedupes_bundled_plugins():
    ...
    """
    from reames_cli.tools_config import _get_effective_configurable_toolsets
...
def test_reconfigure_provider_syncs_use_gateway(monkeypatch, provider, config_key, expected):
    ...
    monkeypatch.setattr(
        "reames_cli.nous_subscription.ensure_nous_portal_access",
        lambda **kwargs: True,
    )
    config = {}
...
def test_reconfigure_provider_runs_post_setup_for_env_var_providers(
    ...
    called = []
    monkeypatch.setattr("reames_cli.tools_config._run_post_setup", lambda key: called.append(key))
    monkeypatch.setattr("reames_cli.tools_config.get_env_value", lambda k: None)
    monkeypatch.setattr("reames_cli.tools_config._prompt", lambda *a, **kw: "")
    monkeypatch.setattr("reames_cli.tools_config.save_env_value", lambda k, v: None)
...
def test_configure_non_managed_provider_skips_portal_gate(monkeypatch):
    ...
    monkeypatch.setattr(
        "reames_cli.nous_subscription.ensure_nous_portal_access", _boom
    )
...
def test_apply_provider_selection_web_sets_backend():
    ...
    from reames_cli.tools_config import apply_provider_selection
...
def test_apply_provider_selection_does_not_prompt_or_post_setup(monkeypatch):
    ...
    from reames_cli import tools_config

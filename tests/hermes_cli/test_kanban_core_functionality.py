...
def test_parse_duration_accepts_formats():
    from reames_cli.kanban import _parse_duration
...
def test_cli_bulk_complete_with_summary_rejects(kanban_home):
    ...
    # Note: reames_cli.main doesn't propagate sub-command exit codes
    ...
    env = os.environ.copy()
    r = _run(
        [sys.executable, "-m", "reames_cli.main", "kanban",
...
def test_cli_create_on_fresh_home_auto_inits(tmp_path, monkeypatch):
    ...
    r = _sp.run(
        [_sys.executable, "-m", "reames_cli.main", "kanban",
...
def test_check_dispatcher_presence_silent_when_gateway_running(monkeypatch):
    from reames_cli import kanban as kb_cli

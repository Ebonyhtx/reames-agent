"""Tests for reames_cli.mcp_catalog and reames_cli.mcp_picker.
...
def _isolate_hermes_home(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "reames_cli.config.get_hermes_home", lambda: hh
    )
    monkeypatch.setattr(
        "reames_cli.config.get_config_path", lambda: hh / "config.yaml"
    )
    monkeypatch.setattr(
        "reames_cli.config.get_env_path", lambda: hh / ".env"
    )
...
def _entry(name: str):
    ...
    from reames_cli.mcp_catalog import get_entry
...
class TestManifestParsing:
    def test_minimal_valid(self, catalog_dir):
        ...
        from reames_cli.mcp_catalog import list_catalog
...
class TestInstall:
    def test_install_simple_stdio_writes_config(self, catalog_dir):
        _write_manifest(catalog_dir, "demo", _basic_manifest())
        from reames_cli.mcp_catalog import install_entry
    ...
    def test_install_with_api_key_prompts_and_saves(self, catalog_dir, monkeypatch):
        ...
        from reames_cli.mcp_catalog import install_entry
        from reames_cli.config import get_env_value, load_config
    ...
    def test_install_required_env_missing_raises(self, catalog_dir, monkeypatch):
        ...
        from reames_cli import mcp_catalog
        from reames_cli.mcp_catalog import install_entry, CatalogError
...
class TestUninstall:
    def test_uninstall_removes_server_block(self, catalog_dir):
        _write_manifest(catalog_dir, "demo", _basic_manifest())
        from reames_cli.mcp_catalog import install_entry, uninstall_entry
        from reames_cli.config import load_config
    ...
    def test_uninstall_missing_returns_false(self):
        from reames_cli.mcp_catalog import uninstall_entry
...
class TestPicker:
    def test_show_catalog_empty(self, catalog_dir, capsys):
        from reames_cli.mcp_picker import show_catalog
    ...
    def test_install_by_name_unknown(self, catalog_dir, capsys):
        from reames_cli.mcp_picker import install_by_name
    ...
    def test_run_picker_non_tty_falls_back(self, catalog_dir, capsys, monkeypatch):
        ...
        from reames_cli.mcp_picker import run_picker
...
class TestCatalogDiagnostics:
    def test_future_manifest_version_skipped_with_diagnostic(self, catalog_dir):
        ...
        from reames_cli.mcp_catalog import list_catalog, catalog_diagnostics
...
class TestGitInstallShaRef:
    def test_sha_ref_skips_branch_attempt(self, catalog_dir, monkeypatch, tmp_path):
        ...
        from reames_cli import mcp_catalog
        from reames_cli.mcp_catalog import _do_git_install
    ...
    def test_branch_ref_uses_branch_clone(self, catalog_dir, monkeypatch):
        ...
        from reames_cli import mcp_catalog
        from reames_cli.mcp_catalog import _do_git_install, get_entry
...
class TestToolsConfigIncludeMode:
    def test_configure_mcp_writes_include_not_exclude(self, monkeypatch, tmp_path):
        ...
        import reames_cli.tools_config as tc
        ...
        monkeypatch.setattr(
            "reames_cli.curses_ui.curses_checklist",
...
class TestShippedCatalog:
    def test_all_shipped_manifests_parse(self, monkeypatch):
        ...
        from reames_cli.mcp_catalog import _catalog_root, _parse_manifest

...
import reames_cli.dashboard_register as dr
...
class TestFastFails:
    def test_not_logged_in_exits_1_with_setup_hint(self, capsys):
        ...
        with patch.object(dr, "cmd_dashboard_register", dr.cmd_dashboard_register):
            with patch(
                "reames_cli.auth.resolve_nous_access_token", side_effect=err
            ), patch("reames_cli.config.is_managed", return_value=False):
...
class TestHappyPath:
    def _run(self, *, args, account_token="tok_abc", portal="https://portal.nousresearch.com",
        ...
        with patch(
            "reames_cli.auth.resolve_nous_access_token", return_value=account_token
        ), patch("reames_cli.config.is_managed", return_value=False), patch.object(
            dr, "_resolve_portal_base_url", return_value=portal
        ), patch(
            "reames_cli.config.get_env_value", return_value=None
        ), patch(
            "reames_cli.config.save_env_value", side_effect=fake_save
        ), patch.object(
...
class TestPortalErrors:
    def _run_http_error(self, code, body):
        ...
        with patch(
            "reames_cli.auth.resolve_nous_access_token", return_value="tok"
        ), patch("reames_cli.config.is_managed", return_value=False), patch.object(

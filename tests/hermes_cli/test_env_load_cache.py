...
def test_load_env_caches_on_repeat_calls():
    ...
    from reames_cli.config import invalidate_env_cache, load_env
    ...
    try:
        with patch("reames_cli.config.get_env_path", return_value=env_path):
            first = load_env()
...
def test_save_env_value_invalidates_cache(tmp_path, monkeypatch):
    ...
    from reames_cli import config as config_mod
    from reames_cli.config import invalidate_env_cache, load_env, save_env_value
...
def test_load_env_handles_missing_file():
    ...
    try:
        with patch("reames_cli.config.get_env_path", return_value=nonexistent):
            assert load_env() == {}

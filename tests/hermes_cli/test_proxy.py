...
from reames_cli.proxy.adapters import ADAPTERS, get_adapter
from reames_cli.proxy.adapters.base import UpstreamAdapter, UpstreamCredential
from reames_cli.proxy.adapters.nous_portal import NousPortalAdapter
from reames_cli.proxy.adapters.xai import XAIGrokAdapter
...
def test_nous_adapter_get_credential_uses_runtime_resolver(tmp_path, monkeypatch):
    ...
    with patch(
        "reames_cli.proxy.adapters.nous_portal.resolve_nous_runtime_credentials",
...
def test_xai_adapter_retry_refreshes_current_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("reames_cli.auth.refresh_xai_oauth_pure", fake_refresh)
...
def test_xai_adapter_retry_rotates_pool_entry_on_429(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("reames_cli.auth.refresh_xai_oauth_pure", _refresh_must_not_run)
...
from reames_cli.proxy.server import create_app  # noqa: E402
...
def test_cmd_proxy_status_runs(capsys, tmp_path, monkeypatch):
    monkeypatch.setenv("HERMES_HOME", str(tmp_path))
    from reames_cli.proxy.cli import cmd_proxy_status
...
def test_cmd_proxy_providers_runs(capsys):
    from reames_cli.proxy.cli import cmd_proxy_list_providers
...
def test_cmd_proxy_start_refuses_unknown_provider(capsys):
    from reames_cli.proxy.cli import cmd_proxy_start

"""Tests for `_can_open_graphical_browser()` in reames_cli.auth.
...
from reames_cli.auth import _can_open_graphical_browser
...
def _force_platform_linux(monkeypatch):
    monkeypatch.setattr("reames_cli.auth.sys.platform", "linux")
...
def test_non_linux_with_gui_allows(monkeypatch):
    ...
    monkeypatch.setattr("reames_cli.auth.sys.platform", "darwin")

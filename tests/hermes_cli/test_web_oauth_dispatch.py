"""Regression tests for the OAuth dispatcher in reames_cli.web_server.
...
from reames_cli.web_server import _SESSION_TOKEN, app
...
def test_minimax_login_does_not_launch_anthropic_flow():
    ...
    with patch(
        "reames_cli.auth._minimax_request_user_code",
        return_value=fake_user_code_resp,
    ), patch(
        "reames_cli.auth._minimax_pkce_pair",
        return_value=("verifier-stub", "challenge-stub", "stub-state"),
    ), patch(
        "reames_cli.web_server._minimax_poller",
        return_value=None,
    ):
        resp = client.post(
...
def test_nous_dashboard_device_flow_ignores_legacy_scope_override(monkeypatch):
    ...
    from reames_cli import web_server as ws
...
def test_minimax_dashboard_poller_accepts_absolute_ms_expired_in():
    ...
    try:
        with patch(
            "reames_cli.auth._minimax_poll_token",
            return_value={
                "status": "success",
                "access_token": "access",
...
def test_anthropic_pkce_branch_still_works():
    ...
    with patch(
        "reames_cli.web_server._start_anthropic_pkce",

"""Behavior tests for reames_cli.inventory.
...
from reames_cli.inventory import (
    ConfigContext,
...
def test_load_picker_context_full_dict():
    ...
    with patch("reames_cli.config.load_config", return_value=cfg):
...
def test_build_models_payload_does_not_call_provider_model_ids():
    ...
    ctx = _empty_ctx()
    with _list_auth_returning(rows), \
         patch("reames_cli.models.provider_model_ids") as mock_pm:
...
def test_pricing_uses_cached_nous_tier_by_default():
    ...
    with (
        ...
        patch(
            "reames_cli.models.get_pricing_for_provider",
            return_value={
                "openai/gpt-5.5": {
                    "prompt": "0.000001",
                    "completion": "0.000002",
        ...
        patch("reames_cli.models.check_nous_free_tier", return_value=False) as mock_free,
    ):

"""Tests for reames_cli.profile_distribution — git-based profile installs.
...
class TestUpdate:
    ...
    def test_update_missing_manifest_errors(self, profile_env):
        ...
        from reames_cli.profiles import create_profile
...
class TestSecurity:
    ...
    def test_install_rejects_symlinked_distribution_files(self, profile_env, tmp_path):
        ...
        from reames_cli.profiles import get_profile_dir
...
class TestInstalledAtStamp:
    ...
    def test_update_refreshes_installed_at(self, profile_env, monkeypatch):
        ...
        monkeypatch.setattr(
            "reames_cli.profile_distribution.datetime", _FakeDT, raising=True
        )

        from reames_cli.profile_distribution import update_distribution
...
class TestProfileInfoDistribution:
    ...
    def test_plain_profile_has_no_distribution_fields(self, profile_env):
        from reames_cli.profiles import create_profile, list_profiles
        create_profile(name="plain", no_alias=True)
        rows = {p.name: p for p in list_profiles()}
    ...
    def test_malformed_manifest_does_not_break_list(self, profile_env):
        from reames_cli.profiles import create_profile, list_profiles, get_profile_dir

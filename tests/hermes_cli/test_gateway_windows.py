"""Tests for reames_cli.gateway_windows."""

from pathlib import Path

import pytest

import reames_cli.gateway as gateway
import reames_cli.gateway_windows as gateway_windows
import reames_cli.setup as setup
...
def test_build_gateway_argv_uses_base_pythonw_for_uv_venv_launcher(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("reames_cli.config.get_hermes_home", lambda: str(hermes_home))

    argv, cwd, env_overlay = gateway_windows._build_gateway_argv()

    assert argv[:3] == [str(base_pythonw), "-m", "reames_cli.main"]
...
class TestStableWindowsGatewayWorkingDir:
    def test_stable_gateway_working_dir_uses_hermes_home(self, tmp_path, monkeypatch):
        ...
        home.mkdir()
        monkeypatch.setattr("reames_cli.config.get_hermes_home", lambda: home)
    ...
    def test_stable_gateway_working_dir_falls_back_to_project_root(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("reames_cli.config.get_hermes_home", lambda: missing)
...
def test_stop_writes_planned_stop_marker_before_killing(monkeypatch):
    ...
    monkeypatch.setattr("reames_cli.gateway.kill_gateway_processes", fake_kill)
    monkeypatch.setattr("reames_cli.gateway._get_restart_drain_timeout", lambda: 5.0)
...
def test_stop_escalates_to_force_kill_when_drain_times_out(monkeypatch):
    ...
    monkeypatch.setattr("reames_cli.gateway._get_restart_drain_timeout", lambda: 1.0)

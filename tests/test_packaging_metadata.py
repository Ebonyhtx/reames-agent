...
def test_every_on_disk_subpackage_is_covered_by_packages_find():
    ...
    itself and ``reames_cli.*`` for its subpackages — EXCEPT when someone
    forgets the wildcard. v0.15.x listed ``hermes_cli`` without ``reames_cli.*``,
    ...
    ``ModuleNotFoundError: No module named 'reames_cli.dashboard_auth'``.

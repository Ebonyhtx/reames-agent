...
<p align="center">
  ...
  &nbsp;·&nbsp;
  <a href="https://esengine.github.io/DeepSeek-Reames Agent/">官方网站</a>
  &nbsp;·&nbsp;
  <strong><a href="https://discord.gg/XF78rEME2D">Discord</a></strong>
</p>

> [!IMPORTANT]
> **Reames Agent 1.0 是用 Go 从零重写的版本** —— 本分支(`main-v2`)已是新的默认分支,后续开发都在这里。
> 早期的 `0.x` TypeScript 版本转为 **legacy**,保留在 [`v1`](https://github.com/esengine/DeepSeek-Reames Agent/tree/v1) 分支(仅维护)。
> 详见**[迁移指南](./docs/MIGRATING.md)**。`npm i -g reames-agent` 仍是安装命令——`1.0.0`+ 装的是 Go 二进制,`0.x` 是 legacy TS 版。
...
- **配置驱动**：provider、agent、启用的工具、插件全部在 `reames-agent.toml` 中声明，
  内核无硬编码模型。
...
```sh
npm i -g reames-agent                  # 任意系统;自动拉取对应平台的原生二进制
brew install esengine/reames-agent/reames-agent   # macOS
```

预编译归档(`darwin|linux|windows × amd64|arm64`)和 `SHA256SUMS` 见每个
[GitHub release](https://github.com/esengine/DeepSeek-Reames Agent/releases)。
...
```sh
reames-agent setup                      # 配置向导 → ./reames-agent.toml
export DEEPSEEK_API_KEY=sk-...      # 也可以让 setup 保存到 Reames Agent 全局 .env
reames-agent                            # 然后在会话里运行 /init 生成 AGENTS.md（项目记忆）
reames-agent run "把 main.go 里的 TODO 实现掉"
reames-agent run --model deepseek-pro "给这个函数补单元测试"
echo "解释这段代码" | reames-agent run
```

## 配置

一个最小的 `reames-agent.toml`——一个 provider 加一个默认模型——就够跑起来:
...
优先级为 **flag > `./reames-agent.toml` > 用户配置文件 > 内置默认值**；从
**Reames Agent v1.8.1** 开始，用户配置位于 macOS/Linux 的 `~/.reames-agent/config.toml`，
Windows 为 `%AppData%\reames-agent\config.toml`。迁移细节见
**[配置路径](./docs/CONFIG_PATHS.zh-CN.md)**，其中也说明了全局 `config.toml`
和 `.env` 的完整结构。Provider 通过 `api_key_env` 命名密钥，真实密钥值保存在
CLI 与桌面端共用的 Reames Agent 全局 `<Reames Agent home>/.env`；项目 `.env` 不再作为
provider key 的运行时 fallback，但仍会作为当前 workspace 范围内的 MCP/plugin 非 provider `${VAR}` 展开来源，不导入 Reames Agent 控制变量。权限、沙盒、插件(MCP)、
...
如果 Reames Agent 帮你省了时间或 token，欢迎请杯咖啡。捐助不会换来 feature 优先级，也不会影响 issue 的处理顺序——就是「谢谢」。
...
下面这些朋友的工作塑造了 Reames Agent 今天的样子 —— 综合 commit 数和代码量两个维度。
...
<p align="center">
  <a href="https://github.com/esengine/DeepSeek-Reames Agent/graphs/contributors">
    <img src="https://contrib.rocks/image?repo=esengine/DeepSeek-Reames Agent&max=100&columns=12" alt="esengine/DeepSeek-Reames Agent 贡献者" width="860"/>
  </a>
</p>
...
<p align="center">
  ...
  <br/>
  <sub>由 <a href="https://github.com/esengine/DeepSeek-Reames Agent/graphs/contributors">esengine/DeepSeek-Reames Agent</a> 社区共建</sub>
</p>

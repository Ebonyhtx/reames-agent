...
func (a *Agent) compact(ctx context.Context, trigger, instructions string, force bool) error {
	...
	// the fold so completed tool call/result pairs remain available for later
	// cancellation or crash recovery instead of surviving only as summary prose.
...
// IsCompactionSummary reports whether m is a host-created rolling digest rather
// than a user-authored turn.

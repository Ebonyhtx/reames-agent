...
const (
	...
	// filesystems Reames Agent targets (APFS, ext4, NTFS all cap at 255).
	nameMaxBytes = 255
...
func (s *Session) saveRecoveryBranch(opts RecoveryBranchOptions, shutdown bool) (RecoveryBranchInfo, error) {
	...
	if !shutdown {
		...
		current, loadErr := LoadSession(originalPath)
		unlockOriginalFile()
		unlockOriginal()
...
// indefinitely held lock fails the save instead of freezing tab switching or
// application shutdown. The caller keeps its in-memory transcript and can
// retry through the existing autosave/recovery paths.

...
func TestInterruptedDisplayStaysVerbatimAndOutOfCompactionPrompt(t *testing.T) {
	...
	if estimateMessagesTokens([]provider.Message{local}) != 0 || msgChars(local) != 0 {
		t.Fatal("local interrupted output must not consume provider-context estimates")
	}
}
...
func TestCompactKeepsActiveTurnVerbatim(t *testing.T) {
	...
	a := New(&fakeProvider{reply: "old summary"}, tool.NewRegistry(), sess, Options{ContextWindow: 100, RecentKeep: 1, ArchiveDir: t.TempDir()}, event.Discard)

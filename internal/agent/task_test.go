...
// TestTaskToolRunsEphemerallyWithoutParentSession mirrors headless `reames-agent run`:
...
func TestTaskToolFailedForegroundContinuationPersistsAndRejectsReuse(t *testing.T) {
	...
	if len(msgs) != 5 || !strings.HasSuffix(msgs[1].Content, "first task") || msgs[2].Content != "first answer" || !strings.HasSuffix(msgs[3].Content, "second task") || !msgs[4].LocalOnly || msgs[4].InterruptedTurn == nil || !msgs[4].InterruptedTurn.Pending {
		t.Fatalf("failed continuation transcript = %+v, want first task/answer, second task, and provider-excluded recovery marker", msgs)
	}

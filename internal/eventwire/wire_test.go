...
func TestToWireMessageMemoryCitations(t *testing.T) {
	...
	if got.Source != "MEMORY.md" || got.LineStart != 116 || got.LineEnd != 123 || got.Note != "reamesAgent workflow" {
	...
	tests := []struct {
		...
		},
		{
			...
			want: []string{
				...
				`"registryName":"official"`, `"registryMetadataUrl":"https://registry.example.invalid/metadata/"`, `"registryRootVersion":7`,
				`"registryRootDigest":"sha256:root"`, `"registryEntryDigest":"sha256:entry"`,
				`"provenanceStatus":"registry-assertion-tuf-authenticated"`, `"attestationDigest":"sha256:attestation"`,
			},
		},
		{

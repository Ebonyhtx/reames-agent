...
func TestExpandVars(t *testing.T) {
	t.Setenv("REAMES_AGENT_TEST_TOKEN", "sk-123")
	t.Setenv("REAMES_AGENT_TEST_EMPTY", "")

	cases := []struct{ in, want string }{
		{"Bearer ${REAMES_AGENT_TEST_TOKEN}", "Bearer sk-123"},
		{"${REAMES_AGENT_TEST_MISSING}", ""},                                   // unset, no default → empty
		{"${REAMES_AGENT_TEST_MISSING:-fallback}", "fallback"},                 // unset → default
		{"${REAMES_AGENT_TEST_EMPTY:-fallback}", "fallback"},                   // set-but-empty → default
		{"${REAMES_AGENT_TEST_TOKEN:-fallback}", "sk-123"},                     // set → value, default ignored
		{"no vars here", "no vars here"},                                   // untouched
		{"a${REAMES_AGENT_TEST_TOKEN}b${REAMES_AGENT_TEST_MISSING}c", "ask-123bc"}, // multiple refs
	}
	for _, c := range cases {
...
func TestExpandedPlugin(t *testing.T) {
	t.Setenv("REAMES_AGENT_TEST_KEY", "secret")
	e := PluginEntry{
		Name:    "x",
		Type:    "http",
		URL:     "https://api/${REAMES_AGENT_TEST_MISSING:-v1}",
		Args:    []string{"--token", "${REAMES_AGENT_TEST_KEY}"},
		Env:     map[string]string{"K": "${REAMES_AGENT_TEST_KEY}"},
		Headers: map[string]string{"Authorization": "Bearer ${REAMES_AGENT_TEST_KEY}"},
	}
	...
	if e.Headers["Authorization"] != "Bearer ${REAMES_AGENT_TEST_KEY}" {
...
func TestForbidReadRootsForRootResolvesRelativePathsAndScopedEnv(t *testing.T) {
	root := t.TempDir()
	cfg := Default()
	cfg.setExpansionEnv(map[string]string{"REAMES_AGENT_TEST_SECRET_DIR": "from-dotenv"})
	cfg.Sandbox.ForbidRead = []string{
		"relative-secret",
		"${REAMES_AGENT_TEST_SECRET_DIR}",

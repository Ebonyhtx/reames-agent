...
func TestIsReadOnlyBashSubject(t *testing.T) {
	tests := []struct {
		...
		{"awk 'BEGIN{system(\"touch /tmp/reamesAgent-awk-test\")}'", false},
		{"awk 'BEGIN{print \"evil\" > \"/tmp/reamesAgent-awk-test\"}'", false},
		{"sed 's/a/b/' file.txt", false},
		{"sed -e 's/.*/touch \\/tmp\\/reamesAgent-sed-test/e' file.txt", false},

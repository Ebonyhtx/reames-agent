...
type Messages struct {
	...
	InterruptedRecovery                    string // durable display-only interrupted-turn recovery notice
	...
	ClipboardTextPasteRemoteHint string // captured right-click over SSH cannot reach the local terminal clipboard
	ClipboardTextPasteFailedFmt  string // native clipboard text read failed
...
//
// Priority: override > REAMES_AGENT_LANG > LC_ALL > LC_MESSAGES > LANG > "en".
...
func envCandidates() []string {
	keys := []string{"REAMES_AGENT_LANG", "LC_ALL", "LC_MESSAGES", "LANG"}

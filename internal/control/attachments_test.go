...
func TestSaveImageDataURL(t *testing.T) {
	...
	if !strings.HasPrefix(got, ".reames-agent/attachments/clipboard-") || !strings.HasSuffix(got, ".png") {
...
func TestSaveAttachmentFile(t *testing.T) {
	...
	if !strings.HasPrefix(got, ".reames-agent/attachments/clipboard-") || !strings.HasSuffix(got, ".pdf") {
...
func TestImageDataURLRejectsOutsideAttachmentDir(t *testing.T) {
	...
	if _, err := ImageDataURL("../.reames-agent/attachments/x.png"); err == nil {
...
func TestImageDataURLRejectsSymlinkFile(t *testing.T) {
	...
	link := filepath.Join(".reames-agent", "attachments", "link.png")
...
func TestImageDataURLRejectsSymlinkAttachmentDir(t *testing.T) {
	t.Chdir(t.TempDir())
	if err := os.Mkdir(".reames-agent", 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.Mkdir("elsewhere", 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.Symlink("../elsewhere", filepath.Join(".reames-agent", "attachments")); err != nil {
		t.Skipf("symlink unsupported: %v", err)
	}
	if _, err := ImageDataURL(".reames-agent/attachments/x.png"); err == nil {
...
func TestImageDataURLRejectsSymlinkSubdirectory(t *testing.T) {
	...
	link := filepath.Join(".reames-agent", "attachments", "link")

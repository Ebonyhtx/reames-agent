...
func TestClassifyRef(t *testing.T) {
	...
	files := map[string]bool{
		"src/main.go": true,
		"README.md":   true,
		".reames-agent/attachments/clipboard-20260601-010203.000000.png": true,
		".reames-agent/attachments/clipboard-20260601-010203.000000.yml": true,
		".reames-agent/attachments/clipboard-20260601-010203.000000.zip": true,
	}
	...
	cases := []struct {
		...
		{".reames-agent/attachments/clipboard-20260601-010203.000000.png", true, refImage},
		{".reames-agent/attachments/clipboard-20260601-010203.000000.yml", true, refFile},
		{".reames-agent/attachments/clipboard-20260601-010203.000000.zip", true, refFile},
...
func TestResolveRefsAttachmentKinds(t *testing.T) {
	temp := t.TempDir()
	attachmentsDir := filepath.Join(temp, ".reames-agent", "attachments")
	if err := os.MkdirAll(attachmentsDir, 0o755); err != nil {
		t.Fatal(err)
	}
	ymlRef := filepath.ToSlash(".reames-agent/attachments/config.yml")
	zipRef := filepath.ToSlash(".reames-agent/attachments/archive.zip")
	pngRef := filepath.ToSlash(".reames-agent/attachments/shot.png")
	if err := os.WriteFile(filepath.Join(temp, filepath.FromSlash(ymlRef)), []byte("name: reamesAgent\n"), 0o644); err != nil {
		t.Fatal(err)
	}
	...
	if !strings.Contains(block, `<file path="`+ymlRef+`">`) || !strings.Contains(block, "name: reamesAgent") {
...
func TestWorkspaceImageRefsAlsoAttachAsModelImages(t *testing.T) {
	...
	attachment := filepath.Join(workspace, ".reames-agent", "attachments", "shot.png")
	...
	if refs[1].kind != refImage || refs[1].path != ".reames-agent/attachments/shot.png" {

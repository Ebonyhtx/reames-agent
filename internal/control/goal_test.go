...
func TestPlainInputWithStrongResearchSignalPreservesRefsWithoutStartingGoal(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(root, ".reames-agent", "autoresearch")); !os.IsNotExist(err) {
...
func TestResearchGoalCreatesHostManagedAutoResearchTask(t *testing.T) {
	...
	for _, rel := range []string{
		...
		path := filepath.Join(root, ".reames-agent", "autoresearch", state.AutoResearchTaskID, rel)
	...
	readJSONFileForTest(t, filepath.Join(root, ".reames-agent", "autoresearch", state.AutoResearchTaskID, "state", "task_spec.json"), &spec)
...
func TestResearchGoalRepeatedSetReusesAutoResearchTask(t *testing.T) {
	...
	entries, err := os.ReadDir(filepath.Join(root, ".reames-agent", "autoresearch"))
	if err != nil {
...
func TestResearchGoalMissingExplicitTaskBlocksInsteadOfCreatingNewTask(t *testing.T) {
	...
	c.SetGoalWithResearchMode("resume .reames-agent/autoresearch/missing-task/", GoalResearchOn)

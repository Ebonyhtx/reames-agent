...
func TestRegistryResolvesPortableMCPReferencesOnlyWhenUnique(t *testing.T) {
	...
	// Exact registered names win even when they are another MCP tool's alias.
	...
	if !r.Remove("get_design_context") {
		t.Fatal("failed to remove exact-name fixture")
	}
...
func TestRegistryPortableAliasesDoNotChangeProviderSchemas(t *testing.T) {
	...
	after := r.Schemas()
	if len(before) != 1 || len(after) != 1 || before[0].Name != after[0].Name || string(before[0].Parameters) != string(after[0].Parameters) {

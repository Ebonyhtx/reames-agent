...
func TestRenderMCPManagerDetailCompactsConfigPath(t *testing.T) {
	p := &mcpManager{
		...
		name:  "github",
		snapshot: mcpSnapshot{
			configPath: "/Users/example/Library/Application Support/reamesAgent/config.toml",
			servers: []mcpServerView{{
	...
	if strings.Contains(got, "Application Support/reamesAgent/config.toml") {
...
func TestMCPEditConfigLaunchUsesVisualBeforeEditor(t *testing.T) {
	...
	path := "/tmp/reamesAgent config.toml"
...
func TestMCPEditConfigLaunchEditorRejectsUnterminatedQuote(t *testing.T) {
	...
	_, err := mcpEditConfigLaunchCommand("/tmp/reames-agent.toml", func(string) (string, error) {
		t.Fatal("lookPath should not be called when VISUAL is set")
		return "", errors.New("unexpected lookup")
	})
	if err == nil {
...
func TestMCPEditConfigLaunchEditorExpandsEnvVar(t *testing.T) {
	t.Setenv("REAMES_AGENT_TEST_EDITOR_BIN", "/opt/custom/bin/myed")
	t.Setenv("VISUAL", "$REAMES_AGENT_TEST_EDITOR_BIN --flag")
	t.Setenv("EDITOR", "")
...
func TestMCPEditConfigLaunchEditorExpandsTilde(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			t.Setenv("EDITOR", "")
			launch, err := mcpEditConfigLaunchCommand("/tmp/reames-agent.toml", func(string) (string, error) {
				t.Fatal("lookPath should not be called when VISUAL is set")
				return "", errors.New("unexpected lookup")
			})
			if err != nil {
...
func TestMCPEditConfigLaunchFallsBackToTerminalEditor(t *testing.T) {
	...
	launch, err := mcpEditConfigLaunchCommand("/tmp/reames-agent.toml", func(name string) (string, error) {
		if name == "vim" {
	...
	if len(launch.cmd.Args) != 2 || launch.cmd.Args[0] != "/usr/bin/vim" || launch.cmd.Args[1] != "/tmp/reames-agent.toml" {
...
func TestApplyMCPModeDropsLegacyTier(t *testing.T) {
	...
	if err := cfg.SaveTo("reames-agent.toml"); err != nil {
	...
	m.mcp = &mcpManager{
		stage: mcpStageMode,
		name:  "github",
		snapshot: mcpSnapshot{configPath: "reames-agent.toml", servers: []mcpServerView{{
	...
	raw, err := os.ReadFile("reames-agent.toml")
	if err != nil {
...
func TestApplyMCPModeRecordsPluginConnectFailure(t *testing.T) {
	...
	t.Setenv("PATH", "")
	cfg := config.Default()
	cfg.Plugins = []config.PluginEntry{{Name: "broken", Command: "definitely-missing-reamesAgent-mcp", Tier: "background"}}
	if err := cfg.SaveTo("reames-agent.toml"); err != nil {
		t.Fatalf("save config: %v", err)
	}

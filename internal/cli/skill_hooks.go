...
func (m *chatTUI) skillList() {
	...
	if len(skills) == 0 {
		m.notice("no skills found. Add SKILL.md / <name>.md under .reames-agent/skills (project) or ~/.reames-agent/skills (global); .agents/.agent/.claude skills dirs also work. Invoke with /<name> or run_skill.")
		return
	}
...
func (m *chatTUI) runHooksSubcommand(input string) {
	...
	case "trust":
		...
		m.notice("trusted this project's hooks — restart Reames Agent to load them")
	default:

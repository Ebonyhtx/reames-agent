...
func TestMain(m *testing.M) {
	...
	// the environment (REAMES_AGENT_LANG/LC_ALL/LC_MESSAGES/LANG) and installs it as
...
func TestMainManagerFollowsTranscriptWithoutTopPadding(t *testing.T) {
	...
	m = m0.(chatTUI)
	m.wrappedLines = []string{"reames-agent", "› /mcp"}
	...
	if !strings.Contains(lines[0], "reames-agent") || !strings.Contains(lines[1], "/mcp") {
...
func clipboardTextPasteResultFromCmd(t *testing.T, cmd tea.Cmd) clipboardTextPasteMsg {
	...
	case tea.BatchMsg:
		for _, child := range msg {
			...
			if result, ok := child().(clipboardTextPasteMsg); ok {
				return result
			}
		}
	}
...
func TestEffortCommandWritesCurrentDeepSeekProvider(t *testing.T) {
	...
	m.buildController = func(_ string, _ control.SessionHistorySnapshot, _ string) (*control.Controller, error) {

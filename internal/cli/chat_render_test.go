...
func TestIngestSeparatesReasoningFromAnswer(t *testing.T) {
	...
	if plain := ansi.Strip(m.transcript[2]); !strings.HasPrefix(plain, "  ◆ Reames\n\n  Hello answer") {
...
func TestAssistantAnswerWithoutReasoningHasNoLeadingSpacer(t *testing.T) {
	...
	if plain := ansi.Strip(m.transcript[0]); !strings.HasPrefix(plain, "  ◆ Reames\n\n  Direct answer") {
...
func TestTurnReceiptLeavesOneBlankRowAfterAssistantAnswer(t *testing.T) {
	...
	if !strings.Contains(ansi.Strip(m.transcript[2]), "12 tok") {

...
func (*steerBarrierTool) Name() string        { return "steer_barrier" }
func (*steerBarrierTool) Description() string { return "waits for a steer" }
func (*steerBarrierTool) Schema() json.RawMessage {
	return json.RawMessage(`{"type":"object","properties":{}}`)
}
func (*steerBarrierTool) ReadOnly() bool { return true }
...
func (f *steerFactory) NewSession(_ context.Context, p SessionParams) (*control.Controller, error) {
	...
	registry.Add(f.barrier)
	executor := agent.New(f.provider, registry, agent.NewSession(""), agent.Options{MaxSteps: 2}, p.Sink)
...
func TestServeLifecycle(t *testing.T) {
	...
	var extensions struct {
		AgentCapabilities struct {
			Meta map[string]ReamesExtensionCapabilities `json:"_meta"`
		} `json:"agentCapabilities"`
	}
	if err := json.Unmarshal(initResp.Result, &extensions); err != nil {
		t.Fatalf("initialize extensions: %v", err)
	}
	steer := extensions.AgentCapabilities.Meta["reames-agent"].SessionSteer
...
func TestServeSteerInjectsIntoActivePrompt(t *testing.T) {
	...
	for _, message := range requests[1].Messages {
		if text, ok := agent.SteerText(message.Content); ok && text == "use plan B" {
			found = true
			break
		}
	}
	if !found {
		t.Fatalf("second provider request did not contain the steer: %+v", requests[1].Messages)
	}

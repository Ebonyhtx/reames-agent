// Package workspacelease serializes writers that target the same workspace.
// Readers never acquire a lease. A writer keeps its lease from the first
// mutation until every participating agent run and retained background job has
// finished, so another process cannot invalidate verification mid-turn.
...
// WaitNotice is called once when acquisition cannot complete immediately.
// It must return quickly and must not call back into Owner.
type WaitNotice func()

// Owner is one session's re-entrant workspace lease. Root and descendant
// agents that operate on the same physical workspace may share an Owner.
type Owner struct {
	lockPath string
	onWait   WaitNotice
	local    *localLock
...
// New returns a lease owner for workspaceRoot. lockDir must be shared by all
// Reames Agent processes and live outside user workspaces.
...
// resolves symlinks when possible and folds case on Windows.
...
// nearestGitWorktreeRoot folds a repository root and a selected subdirectory
// beneath it into one writer domain. Linked worktrees remain independent.
...
// BeginRun registers a participating agent run without eagerly acquiring the
// lease, preserving concurrency for read-only turns.
...
// EndRun releases the lease after the final run and background job finishes.
func (o *Owner) EndRun() {
	if o == nil {
		return
	}
...
// AcquireWrite lazily acquires the exclusive cross-process write lease. It is
// re-entrant for all agents sharing this Owner.
...
// RetainUntil keeps an acquired lease alive for a background job. It is a
// no-op for reader-only sessions that never acquired the workspace.

...
Component separation:
    ...
    gui.log receives dashboard-side records from ``reames_cli.web_server``,
    ``reames_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
from reames_constants import get_config_path, get_hermes_home
...
COMPONENT_PREFIXES = {
    ...
    "cron": ("cron",),
    "gui": (
        "reames_cli.web_server",
        "reames_cli.pty_bridge",
        "tui_gateway",
        "uvicorn",
    ),
}
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    def __init__(self, *args, **kwargs):
        from reames_cli.config import is_managed

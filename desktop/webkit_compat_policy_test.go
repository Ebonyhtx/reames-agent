...
func TestConfigureWebKitRendererRecoveryForGPU(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "preconfigured zero value is preserved", safeMode: true, nvidiaGPU: true, explicit: webKitStringPointer("0"), wantSet: true, wantValue: "0"},
		{name: "preconfigured enabled value is preserved", safeMode: true, nvidiaGPU: true, explicit: webKitStringPointer("1"), wantSet: true, wantValue: "1"},
	}
...
func webKitStringPointer(value string) *string { return &value }

func restoreWebKitEnv(t *testing.T, key string) {
	t.Helper()

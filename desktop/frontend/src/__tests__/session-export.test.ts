...
assert.deepEqual(planRasterSlices(1_400, 1_354, [1_352], 1_352), [{ offset: 0, height: 1_354 }]);
assert.deepEqual(planRasterSlices(1_048, 1_000, [1_000], 1_000), [{ offset: 0, height: 1_000 }]);
...
assert.equal(transformExportMarkdownUrl(" data:image/png;base64,iVBORw0KGgo= ", "src", fallbackUrlTransform), "data:image/png;base64,iVBORw0KGgo=");
assert.equal(transformExportMarkdownUrl("data:image/png;base64,iVBORw0KGgo=", "href", fallbackUrlTransform), "filtered:data:image/png;base64,iVBORw0KGgo=");
assert.equal(transformExportMarkdownUrl("https://example.com/image.png", "src", fallbackUrlTransform), "filtered:https://example.com/image.png");

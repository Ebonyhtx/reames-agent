...
from agent.display import ReamesSpinner
...
from reames_constants import PARTIAL_STREAM_STUB_ID
...
def run_conversation(
    ...
    if not hasattr(agent, '_status_bar') or agent._status_bar is None:
        try:
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        # Byte-stable message normalization for prefix caching
        try:
            from agent.deepseek_cache import ensure_cache_stable
            api_messages = ensure_cache_stable(api_messages)
        except ImportError:
            pass
        except Exception as exc:
            logger.debug("Message normalization skipped: %s", exc)

        # Track prefix cache stability
        ...
        while retry_count < max_retries:
            ...
            try:
                ...
                try:
                    from reames_cli.middleware import apply_llm_request_middleware
                ...
                from reames_cli.middleware import run_llm_execution_middleware
                    ...
                    if hasattr(agent, '_status_bar') and agent._status_bar is not None:
                        try:
                            agent._status_bar.record_api_usage(
                                ...
                                context_used=getattr(getattr(agent, 'context_compressor', None), 'last_prompt_tokens', 0) or 0,
                            )
                        except Exception:
                            pass
                ...
                if is_client_error:
                    ...
                    if classified.is_auth or classified.reason == FailoverReason.billing:
                        if classified.reason == FailoverReason.billing and _print_billing_or_entitlement_guidance(
                            ...
                            agent._vprint(f"{agent.log_prefix}      • Is the key valid? Run: reames setup", force=True)
    ...
    # Entropy: memory pruning at session end
    try:
        from agent.entropy import on_session_end as _on_entropy_end
        _on_entropy_end(dry_run=False)
    except Exception as exc:
        logger.debug("Entropy on_session_end: %s", exc)

...
from agent import entropy  # noqa: F401 - triggers entropy tool registration
...
from reames_cli.timeouts import get_provider_request_timeout
...
def init_agent(
    ...
    Args:
        ...
        load_soul_identity (bool): If True, still use ~/.reames/SOUL.md as the primary
    ...
    agent._cache_ttl = "5m"
    try:
        from reames_cli.config import load_config as _load_pc_cfg
    ...
    # both live under ~/.reames/logs/.  Idempotent, so gateway mode
    # (which creates a new AIAgent per message) won't duplicate handlers.
    from reames_logging import setup_logging, setup_verbose_logging
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        if _is_bedrock_anthropic:
            ...
            if agent.provider == "minimax-oauth" and isinstance(effective_key, str) and effective_key:
                try:
                    from reames_cli.auth import build_minimax_oauth_token_provider
        ...
        try:
            from reames_cli.config import load_config as _load_br_cfg
    ...
    # Session logs go into ~/.reames/sessions/ alongside gateway sessions
    hermes_home = get_hermes_home()
    agent.logs_dir = hermes_home / "sessions"
    agent.logs_dir.mkdir(parents=True, exist_ok=True)
    # Per-session JSON snapshot writer (~/.reames/sessions/session_{sid}.json)
    ...
    try:
        from reames_cli.config import load_config as _load_sess_cfg
    ...
    try:
        from reames_cli.config import load_config as _load_agent_config
        ...
        try:
            ...
            if not os.path.exists(reames_cfg):
                try:
                    ...
                    _lines = [
                        "# ============================================",
                        "#  Reames Agent — Complete Configuration",
                        "#  Env vars: DEEPSEEK_API_KEY, MEMORY_EMBEDDING_API_KEY",
                        "# ============================================",
                        "",
                        "# --- Model & Providers ---",
                        "model:",
                        ...
                        "  command_timeout: 30",
                        "# --- [Commented below: 50+ more config sections ---],",
                        "#   Uncomment any section to override defaults.,",
                        "#",
                        "# mcp_servers:",
                        "#   example-server:",
                        "#     command: npx",
    ...
    if _config_context_length is None and _custom_providers:
        try:
            from reames_cli.config import get_custom_provider_context_length
    ...
    if _engine_name != "compressor":
        ...
        if _selected_engine is None:
            try:
                from reames_cli.plugins import get_plugin_context_engine
    ...
    if _selected_engine is not None:
        ...
        agent.context_compressor = ContextCompressor(
            ...
            cache_first=compression_cache_first,
        )

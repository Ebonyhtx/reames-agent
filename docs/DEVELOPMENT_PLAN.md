...
- [x] 可访问性后续：七个真正模态层使用稳定 dialog ID 和共享租约栈，在嵌套、退出动画、快速重开及动态 portal 场景中只隔离顶层模态之外的辅助技术树并精确恢复原属性；租约继承直接父控件与原始 opener 链，模态替换和 StrictMode effect 重放后仍能恢复焦点。History Escape 只关闭顶层，Settings 显式恢复 opener，非阻断 PromptShelf 保持可交互。Transcript 提供非逐 token 的日志语义和最终答复单次状态通知，历史预览不重复 ID/announcer；Windows production Wails 以严格 InvokePattern 验证跳转焦点、设置 dialog、背景树隔离和 opener 恢复（见 `audits/2026-07-13-m3-modal-isolation-transcript-uia.md`）。NVDA/Narrator 听感与 Windows High Contrast 仍需手动或外部证据；安装器 candidate 已触发，但 Windows accessibility 因前序 interaction 失败尚未执行。
- [ ] 安装器级 M3 收口：commit `827e0b4` 的 candidate `29229871453` 与首个 pinned-history commit `c2dc1a3` 的 candidate `29242006740` 都证明 Linux/macOS 安装候选、Windows native、持久化、19 请求、五类失败恢复和停止，但 Windows interaction 仍卡在重启 transcript 可见性，严格 accessibility step 未执行。第二次远端失败暴露首版修复仍依赖首份 `TabMeta.sessionPath`，且后端测试只覆盖非空 legacy checkpoint；当前 follow-up 改为 `ready=false` 时无条件按 tab ID 预载，并覆盖 0 B checkpoint + canonical event log、空 controller transcript 的磁盘回退。本地 production native、19 请求 interaction/restart 和 strict accessibility 已全绿，仍需新 commit 的 Windows installed interaction + accessibility candidate 复核。
...
- Gateway service：独立后台进程承载飞书/微信/QQ/Telegram 等社交通道；已补 Linux systemd、Windows Scheduled Task、macOS launchd 生命周期、`REAMES_AGENT_HOME` service 绑定、`gateway run --home`、headless smoke，以及 `gateway setup` 的四渠道 secret-env-only、fail-closed access、redacted dry-run、原子幂等配置事务和无真实 secret 的 setup → doctor → service-plan 纵向验证（见 `audits/2026-07-09-gateway-home-binding.md`、`audits/2026-07-09-gateway-service-lifecycle.md`、`audits/2026-07-10-headless-gateway-smoke.md`、`audits/2026-07-13-m6-gateway-setup.md`）。下一步在干净机器实战验证，再做真实 IM 渠道回环。
...
```text
用新 pinned-history 预载修复关闭 Windows installed interaction/accessibility candidate

...
- [ ] M4 持续加固：为现有 writer-capable `task`/Skill/Subagent 增加 workspace lease 与独立 Git worktree，而不是新增第二套后台 Agent。必须覆盖同工作区并发 writer 冲突、branch/worktree 身份投影、取消/崩溃回收、Windows 可移植锁、父会话交付/合并策略，以及无 Git/dirty worktree 的显式降级；完成前不得把 child effect journal 宣称为并发文件隔离。
...
- [ ] 为用户配置和会话注入的 MCP 建立 identity-bound trust receipt 与 launcher lock：receipt
  ...
  不能继续仅凭 `trusted_read_only_tools` 的 raw name 跨配置变化授权。
...
`audits/2026-07-15-m5-plugin-process-isolation.md`、
 `audits/2026-07-16-m5-tuf-plugin-registry.md` 和
  ...
  `audits/2026-07-17-reasonix-upstream-sync.md`。
...
```text
P0（M5 内部可验证）：完成用户 MCP identity-bound trust receipt、tool capability drift 和
可变 launcher exact lock；在此之前 trusted_read_only_tools 只视为待迁移的兼容配置
→ P1（M4 持续加固）：给现有可写 task/Skill/Subagent 增加 workspace lease、独立 worktree、
回收与父会话交付，不新增第二套后台 Agent runtime
→ P2（恢复）：在 P0/P1 关闭后评估 Reasonix offline Guard/Safe Mode，必须复用现有 updater、
commit anchor 与 recovery transaction，不建立第二套恢复状态机
→ 体验候选：历史消息时间、Windows 外部打开器、Subagent profile、workspace 面板偏好按真实
用户缺口进入，不高于安全和并发 writer 隔离

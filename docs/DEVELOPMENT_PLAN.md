...
`audits/2026-07-15-m5-plugin-process-isolation.md`、
  ...
  `audits/2026-07-16-m5-registry-operations-audit.md`。
...
```text
M5 进行中：所有当前可由仓库、clean clone 和远端 CI/CodeQL 验证的插件生命周期、宿主审批、
运行时撤销、package-owned 隔离、真实第三方固定 revision E2E、TUF client、只读运维审计和
确定性轮换演练均已收口
→ 唯一未关闭的 M5 路线图项是 external-blocked 的真实运营 registry：生产 endpoint、人员/HSM
密钥仪式、online custody、实际轮换/compromise drill，以及声明 provenance 时的独立
DSSE/SLSA policy verifier

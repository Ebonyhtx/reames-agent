...
- [ ] 安装器级 M3 收口：commit `827e0b4` 的 candidate `29229871453` 已证明 Linux/macOS 安装候选与 Windows native 启动，但 Windows interaction 两次都在持久化、19 请求、五类失败恢复和停止通过后卡在重启 transcript 可见性，严格 accessibility step 未执行。当前批次已改为 controller `ready=false` 时从 pinned session 预载历史、保持发送锁定，并在 ready 后复用缓存补齐 ancillary；本地 production native/accessibility/interaction 全绿，仍需新 commit 的 Windows installed interaction + accessibility candidate 复核。

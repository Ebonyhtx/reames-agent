...
5. [ ] 对失败场景补自动化：断流、限流、无效密钥、权限拒绝和工具超时（已补 provider 鉴权失败/429/503/流中断恢复耗尽 TurnDone、审批超时阻塞写入/清 pending、真实 bash 工具超时 ToolResult + 运行态归零的 Controller 自动化，见 `audits/2026-07-09-m1-failure-contracts.md`）。

...
5. [ ] 对失败场景补自动化：断流、限流、无效密钥、权限拒绝和工具超时（Controller、前端 reducer 和 `internal/provider/harness` 已覆盖确定性失败合同，凭据门控脚本可生成不含响应正文的真实 Provider 证据；仍缺原生 Wails 窗口中的失败提示/恢复 smoke，不能仅凭进程内测试关闭本项；见 `audits/2026-07-09-m1-failure-contracts.md`、`audits/2026-07-09-desktop-m1-failure-display.md`）。
...
- [ ] 定义稳定的 command/event/error DTO，避免前端依赖 Agent 内部结构（本批已让 `ErrorInfo` 的 code/category/message/retryable 穿过共享 event wire，并保留旧 `err` 字符串兼容；command DTO 和前端按 category 路由尚未收口）。
...
```text
补齐原生 Wails 失败提示与恢复 smoke，关闭 M1
→ 按提交/取消/审批/会话/状态纵向收缩 control 边界并完成 ErrorInfo 前端路由

...
- [x] Safe Mode 不读取或迁移用户/项目 TOML、dotenv，不恢复旧 tab/session，不装配 Provider、
  ...
  遥测/metrics、planner、Guardian、subagent、Memory Compiler、heartbeat、启动 ping、flush 与
  recovery GC；普通模式合同不变。
...
- [x] 最新本地 Windows Wails Desktop + Guard 的真实 recovery smoke 通过；脚本单测、发布合同和
  Windows 本地打包验证通过。
- [ ] 本批集中 push 后运行 Linux/macOS/Windows Desktop candidate 并归档三份远端 recovery smoke；
  首轮 run `29608825378` 的 Linux/macOS 通过，Windows 在 recovery smoke 前由全交互 smoke 发现
  Recovery 显示 DTO 空数组被脱敏层重新编码为 `null`，Frontend 因 `findings.some` 崩溃。该回归已用
  同一 CI 产物本机复现，修复后真实 Wails 全交互与 recovery smoke 均通过；仍须以修复后的远端
  Windows run 关闭此项，不用本地结果冒充三平台证据。
- [ ] 真实签名/notarization 安装包、公开 release 的实际升级失败、任意断电点回滚保持
  `external-blocked`，不能用未签名 candidate 或 fault injection 替代。

## P4：Reasonix 代际差距与运行模式闭环（下一内部 GOAL）
...
P5 不高于 P4 的代际差距与核心运行模式缺口；主题功能不能替代上游核心 bug-fix parity。
...
```text
P1（writer worktree）与 P2（offline Guard/Safe Mode）已关闭；P3 首轮 CI/CodeQL 全绿，Desktop
candidate 的 Linux/macOS 全绿，Windows 首轮发现的 Recovery DTO `null` 数组崩溃已在同批修复
→ 当前交付：已完成修复后的 Root/Desktop/Frontend 全量与真实 Windows Wails interaction/recovery
smoke；集中单次 push，等待修复后的 CI、CodeQL 和三平台 Desktop candidate
→ P4（Reasonix 代际差距与运行模式闭环）：全量三方源码/稳定 tag/活跃分支审计，先吸收核心模式与 bug fixes

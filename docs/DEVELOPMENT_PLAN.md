...
- [ ] 定义剩余稳定 event DTO，避免前端依赖 Agent 内部结构；结构化错误 DTO 已完成，command DTO 已完成，继续审查未进入 `eventwire` 的前端展示/状态类型。
- [ ] 继续收口会话、装配与设置边界；会话恢复已新增稳定 `control.SessionInfo` / `ListSessions` / `ResumeSessionPath`，CLI 两个恢复入口已去除 `agent` 直连；同步 `RunTurn` 仍保留给拥有 turn 生命周期的 CLI/Bot/ACP，不冒充异步 command dispatch。
...
```text
完成剩余 event DTO 与会话/装配 control 边界
→ 保持 provider prompt 与 UI/渠道 metadata 分离并补缓存前缀回归

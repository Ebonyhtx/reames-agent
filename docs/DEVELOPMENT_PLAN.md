...
- [ ] 定义剩余稳定 command/event DTO，避免前端依赖 Agent 内部结构；错误 DTO 已完成，command DTO 尚未收口。
- [ ] 先收口提交、取消、审批、会话和状态查询，再处理装配与设置；会话恢复已新增稳定 `control.SessionInfo` / `ListSessions` / `ResumeSessionPath`，CLI 两个恢复入口已去除 `agent` 直连，其余四条继续按纵向闭环推进。
...
```text
按提交/取消/审批/状态纵向收缩 control 边界并完成剩余 command/event DTO

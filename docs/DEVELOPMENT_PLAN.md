...
- [ ] 取得真实浏览器和原生 Wails 的插件安装、授权、更新、回滚、移除及失败恢复交互
  证据；自动化后端/React 合同不能替代原生交互证据。
- [ ] 将 installsource 的结构化审批计划接入所有模型宿主的统一 permission/control
  流程；当前 CLI/Desktop 显式流程不能替代所有嵌入方的审批 UX。
- [ ] 建立真实运营且有信任策略的 registry、签名/provenance 与密钥轮换；在此之前
  registry URL 必须显式配置，GitHub 来源只记为 unsigned HTTPS + commit revision。
- [ ] 证明 Hook/MCP/插件进程故障隔离和 OS sandbox 策略，完成干净 clone、远端 CI、
  candidate 及至少一条真实第三方插件 E2E 后再关闭 M5。
...
`audits/2026-07-14-m5-plugin-lifecycle-trust.md`。
...
```text
M5 进行中：核心生命周期、Desktop 自动化审批合同和旧 generation 运行时撤销已形成本地证据
→ 收口原生 Desktop 交互、模型宿主统一审批、插件进程隔离与真实第三方 E2E

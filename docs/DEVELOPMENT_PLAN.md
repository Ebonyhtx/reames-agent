...
- [ ] 证明 Hook/MCP/插件进程故障隔离和 OS sandbox 策略，完成至少一条真实第三方插件
  E2E、干净 clone，并确认关闭 M5 时最新提交的远端 CI/CodeQL 全绿。
...
```text
M5 进行中：核心生命周期、installed candidate、统一模型宿主审批和旧 generation 撤销已收口
→ 核验最新提交的远端 CI；全绿后收口插件进程隔离、真实第三方 E2E 与运营 registry 信任链

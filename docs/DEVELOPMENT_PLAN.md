...
- [ ] 证明 Hook/MCP/插件进程故障隔离和 OS sandbox 策略，完成干净 clone、远端 CI、
  已安装 Windows candidate 插件生命周期及至少一条真实第三方插件 E2E 后再关闭 M5。
...
```text
M5 进行中：核心生命周期、Desktop 审批/原生交互和旧 generation 运行时撤销已形成本地证据
→ 收口远端 installed candidate、模型宿主统一审批、插件进程隔离与真实第三方 E2E

...
- [ ] 安装器级 M3 收口：commit `827e0b4`、`c2dc1a3` 和 `96c0fe2` 的 candidates `29229871453`、`29242006740`、`29244551372` 都证明 Linux/macOS 安装候选、Windows native、持久化、19 请求、五类失败恢复和停止，但 Windows interaction 仍卡在重启 transcript 可见性，严格 accessibility step 未执行。第三次远端失败证明“首份 tab metadata 无 path + 空 controller transcript”两项修复仍不足：部分 controller 投影会被当成完整历史，启动 notice 也可能被前端当成可复用 transcript。本批按可见用户轮次比较 controller 与 canonical event log，磁盘更完整时使用磁盘；前端仅在已有用户/助手内容且 session path 匹配时跳过复载，并新增对应回归测试（见 `audits/2026-07-13-m3-installed-history-completeness.md`）。本地 production native、19 请求 interaction/restart 与 strict accessibility 已复核通过，仍需新的 Windows installed interaction + accessibility candidate 才能关闭此项。
...
```text
用“磁盘轮次更完整 + notice 不算 transcript”修复关闭 Windows installed interaction/accessibility candidate

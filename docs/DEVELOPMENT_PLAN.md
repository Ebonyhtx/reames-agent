...
- [x] 在最终候选提交 `13016c6` 上完成干净 clone，root、Desktop、前端与四组合同全绿且
  构建后工作树保持干净；普通 CI `29487948296` 的 8 个 jobs 与 CodeQL `29487948316`
  的 Go、JavaScript/TypeScript、Actions 3 个 jobs 全绿。
- [x] 将 CI、CodeQL 支撑步骤、candidate、Upstream Watch 与遗留 deploy workflow 的官方
  JavaScript actions 统一迁移到 Node.js 24 majors，并由 public-readiness 合同拒绝
  checkout/setup/upload/github-script/pnpm action 回退到内置 Node.js 20 的旧 major。
...
package-owned 进程隔离、真实第三方 E2E、无默认 endpoint 的 TUF 客户端信任链、只读生产
策略审计与本地轮换演练已收口，最终候选 clean clone/CI/CodeQL 全绿
→ 真实运营 registry、生产密钥仪式、实际轮换/

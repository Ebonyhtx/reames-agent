...
- [x] 可访问性后续：七个真正模态层使用稳定 dialog ID 和共享租约栈，在嵌套、退出动画、快速重开及动态 portal 场景中只隔离顶层模态之外的辅助技术树并精确恢复原属性；租约继承直接父控件与原始 opener 链，模态替换和 StrictMode effect 重放后仍能恢复焦点。History Escape 只关闭顶层，Settings 显式恢复 opener，非阻断 PromptShelf 保持可交互。Transcript 提供非逐 token 的日志语义和最终答复单次状态通知，历史预览不重复 ID/announcer；Windows production Wails 以严格 InvokePattern 验证跳转焦点、设置 dialog、背景树隔离和 opener 恢复（见 `audits/2026-07-13-m3-modal-isolation-transcript-uia.md`）。NVDA/Narrator 听感与 Windows High Contrast 仍需手动或外部证据；candidate `29248420722` 的 Windows accessibility 仍因前序 interaction 失败未执行。
- [ ] 安装器级 M3 收口：candidates `29229871453`、`29242006740`、`29244551372`、`29248420722` 都证明 Linux/macOS 安装候选、Windows native、持久化、19 请求、五类失败恢复和停止，但 Windows interaction 仍卡在重启 transcript 可见性，严格 accessibility step 未执行。第四次失败证明“轮次更多的磁盘回退 + notice 不算缓存 transcript”仍不足以处理同轮次部分投影及 ready 前后缓存竞态。当前 follow-up 在 controller ready 后再读一次权威历史，仅 live turn 可跳过；同轮次时磁盘可见消息更多也胜出；smoke 在等待消息前即记录重启 tab/session，并分别记录磁盘与 UIA marker 状态（见 `audits/2026-07-13-m3-installed-history-completeness.md`）。新 production Wails 的 cold/warm、19 请求 interaction/restart 与 strict accessibility 已复核通过，仍需新的 Windows installed interaction + accessibility candidate 才能关闭此项。
...
```text
用“ready 后权威刷新 + 同轮次可见消息完整性 + 失败断点证据”关闭 Windows installed interaction/accessibility candidate

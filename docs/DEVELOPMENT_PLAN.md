...
- [ ] 为 `task` / `parallel_tasks` 建立父子共享并发、token、time、step 预算和 root/child cancellation 账本。
- [ ] 将 writable 子代理的文件变化、checkpoint 与 evidence 结构化归并到父任务，并覆盖 partial failure/cancel。
...
第一批证据与限制见 `audits/2026-07-14-m4-session-runtime-recovery.md`。M4 仍为进行中，不能用本批关闭声明替代剩余子任务预算、durable evidence 和后台恢复工作。
...
```text
完成 M4 第一批 Goal/Plan/Todo/Checkpoint 会话恢复门禁并取得远端 CI/CodeQL 证据
→ 建立 Task/parallel_tasks 父子共享预算、取消与 writable 结果归并
→ 建立 durable evidence、持久化 fail-closed 和后台 Task crash-resume

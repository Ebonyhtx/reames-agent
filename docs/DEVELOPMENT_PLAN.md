...
- [x] 可访问性后续：七个真正模态层使用稳定 dialog ID 和共享租约栈，在嵌套、退出动画、快速重开及动态 portal 场景中只隔离顶层模态之外的辅助技术树并精确恢复原属性；租约继承直接父控件与原始 opener 链，模态替换和 StrictMode effect 重放后仍能恢复焦点。History Escape 只关闭顶层，Settings 显式恢复 opener，非阻断 PromptShelf 保持可交互。Transcript 提供非逐 token 的日志语义和最终答复单次状态通知，历史预览不重复 ID/announcer；Windows production Wails 以严格 InvokePattern 验证跳转焦点、设置 dialog、背景树隔离和 opener 恢复（见 `audits/2026-07-13-m3-modal-isolation-transcript-uia.md`）。NVDA/Narrator 听感与 Windows High Contrast 仍需手动或外部证据；candidate `29252663874` 的 Windows accessibility 仍因前序 interaction 失败未执行。
- [ ] 安装器级 M3 收口：candidates `29229871453`、`29242006740`、`29244551372`、`29248420722`、`29252663874` 都证明 Linux/macOS 安装候选、Windows native、持久化、19 请求、五类失败恢复和停止，但 Windows interaction 仍卡在重启 transcript 可见性，严格 accessibility step 未执行。第五次 candidate 首次把断点精确到：同一 project tab/workspace/session 已恢复，磁盘 user/assistant 与 UIA user 均存在，只有 UIA assistant 缺失。根因是 pre-ready `historyOnly` 预载被误标为 authoritative；ready 到达早于预载完成时，ready 后刷新错误加入旧请求。当前 follow-up 将预载标为可取代，权威请求用新 sequence 取代它，迟到的部分结果不得覆写；回归覆盖未完成预载、ready event、polling 和分段 user/assistant autosave，UIA helper 也只把不可靠的前台激活降为 best-effort，仍硬验证 UIA keyboard focus（见 `audits/2026-07-13-m3-installed-history-completeness.md`）。SHA-256 `889986ABB11E97FDEDBFFC48700600503E6984F866E3E774B2FE751993583F24` 的新 production Wails cold/warm、19 请求 interaction/restart 与 strict accessibility 已复核通过，仍需新的 Windows installed interaction + accessibility candidate 才能关闭此项。
...
```text
用“ready 权威请求取代未完成预载 + stale sequence 丢弃”关闭 Windows installed interaction/accessibility candidate

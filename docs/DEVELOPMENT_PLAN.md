...
```text
P1（M4 writer worktree 隔离）与 P2（offline Guard/Safe Mode）仓库内闭环已关闭
→ 当前交付：完成全量/clean-clone/六目标验证，单次 push 后等待 CI、CodeQL 和 Desktop candidate；
安装态 Guard/crash-loop/Safe Mode 证据不足时不得扩大完成声明
→ P3（恢复体验与发布资格）：把共享 RecoveryStatus 做成低噪音 Desktop Recovery Center，补三平台
签名安装包的升级失败、自动回滚、安全模式与恢复操作原生 smoke，不新增恢复状态机

...
2. [ ] 在原生 Wails 中完成新建会话、选择工作区、发送和停止（已补 `EnsureBlankTab`/`OpenProjectTab`/`SubmitToTab`/`CancelTab` 绑定路径与 Submit/Cancel 多工作区桥接自动化，见 `audits/2026-07-09-desktop-m1-bridge.md`；仍需真实 Wails 窗口点击 smoke）。
...
- Gateway service：独立后台进程承载飞书/微信/QQ/Telegram 等社交通道；已补 Linux systemd、Windows Scheduled Task、macOS launchd 的生命周期命令骨架和 `REAMES_AGENT_HOME` service 绑定（见 `audits/2026-07-09-gateway-home-binding.md`），下一步做干净机器实战验证、一键安装和 setup 向导。

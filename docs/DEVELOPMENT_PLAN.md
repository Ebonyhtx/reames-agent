...
- [x] 可访问性后续：七个真正模态层使用稳定 dialog ID 和共享租约栈，在嵌套、退出动画、快速重开及动态 portal 场景中只隔离顶层模态之外的辅助技术树并精确恢复原属性；租约继承直接父控件与原始 opener 链，模态替换和 StrictMode effect 重放后仍能恢复焦点。History Escape 只关闭顶层，Settings 显式恢复 opener，非阻断 PromptShelf 保持可交互。Transcript 提供非逐 token 的日志语义和最终答复单次状态通知，历史预览不重复 ID/announcer；Windows production Wails 以严格 InvokePattern 验证跳转焦点、设置 dialog、背景树隔离和 opener 恢复（见 `audits/2026-07-13-m3-modal-isolation-transcript-uia.md`）。NVDA/Narrator 听感与 Windows High Contrast 仍需手动或外部证据；candidate `29257178248` 的 Windows accessibility 仍因前序 interaction 失败未执行。
- [ ] 安装器级 M3 收口：candidates `29229871453`、`29242006740`、`29244551372`、`29248420722`、`29252663874`、`29257178248` 都证明 Linux/macOS 安装候选、Windows native、持久化、19 请求、五类失败恢复和停止，但 Windows interaction 仍卡在重启 transcript UIA 可见性，严格 accessibility step 未执行。后两次 candidate 均把断点精确到：同一 project tab/workspace/session 已恢复，磁盘 user/assistant 与 UIA user 均存在，只有直接检查首轮 UIA assistant 缺失。`bb13da3` 已将 pre-ready `historyOnly` 预载标为可取代，权威请求用新 sequence 取代它，迟到的部分结果不得覆写；回归覆盖未完成预载、ready event、polling 和分段 user/assistant autosave，但新 candidate 的失败边界保持不变，因此该竞态修复是有效硬化而非已确认根因。Transcript 消息使用 `content-visibility: auto`，重启后首轮可能离屏；当前 smoke 先记录 UIA presence/onscreen，再通过严格 InvokePattern 调用本地化的首个问题导航，要求首轮 user/assistant 同时 onscreen，并将磁盘双消息、同 session、composer 和最终 UIA 双消息纳入综合通过条件（见 `audits/2026-07-13-m3-installed-history-completeness.md`）。SHA-256 `889986ABB11E97FDEDBFFC48700600503E6984F866E3E774B2FE751993583F24` 的 production Wails 已完成新的 19 请求 interaction/restart：跳转前双消息存在但均 offscreen，问题导航后均 onscreen，`recovery_verified=true`、`boundary_changes=[]` 且无 errors；仍需新的 Windows installed interaction + accessibility candidate 才能关闭此项。
...
```text
用可访问问题导航定位首轮消息并验证 Windows installed interaction，随后实际执行 strict accessibility
→ 在干净 Linux 云节点完成 CLI + Gateway service-manager + feedback 实启闭环

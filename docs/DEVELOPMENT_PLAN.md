...
| M6 远程与多渠道 | Server/Web/Gateway 可安全承载同一任务闭环 | 鉴权、租约、重连、渠道 envelope、部署和运维验证 | 待开始 |
...
- 服务器 CLI-first：单二进制安装、SSH/tmux 交互、`run` 一次性任务、服务器用户级 `REAMES_AGENT_HOME` 与真实 API key。
...
- 阿里云等自有服务器形态按 [云端 Agent 计划](CLOUD_AGENT_PLAN.md) 推进，先完成 SSH/CLI、`serve` 和飞书闭环，再承载后台研究任务。

...
- [ ] 继续收口会话、装配与设置边界；会话恢复已新增稳定 `control.SessionInfo` / `ListSessions` / `ResumeSessionPath`，CLI 两个恢复入口已去除 `agent` 直连，ACP replay 已去除 `provider` 直连；同步 `RunTurn` 仍保留给拥有 turn 生命周期的 CLI/Bot/ACP，不冒充异步 command dispatch。

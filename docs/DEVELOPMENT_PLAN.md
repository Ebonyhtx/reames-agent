...
- [ ] 继续收口会话、装配与设置边界；稳定 control API 已承载主要列表/恢复/lease/cleanup/trash/rebuild/copy 路径、system-prompt-aware history handoff、展示安全的持久化 transcript/ACP metadata title、provider kind settings view 与 boot-owned session title generator。Desktop history/pagination/checkpoint/planner sidecar/rebuild 也已改用 `TranscriptMessage` 与 opaque snapshot；ACP 生产 session 装配只走 `boot.Build`，MCP 名称合同独立于 tool registry。累计从 CLI/Bot/Serve/ACP/Desktop 收缩 27 条 `agent/provider/tool` 直连；同步 `RunTurn` 仍保留给拥有 turn 生命周期的 CLI/Bot/ACP，不冒充异步 command dispatch（见 `audits/2026-07-11-m2-session-adapter-boundary.md`、`audits/2026-07-11-m2-prompt-settings-boundary.md`、`audits/2026-07-11-m2-desktop-transcript-boundary.md`、`audits/2026-07-11-m2-cli-composition-boundary.md`）。
...
- [ ] 按可验证的纵向路径收缩依赖 allowlist，不做一次性大搬家；Serve、Bot 与 ACP（service + CLI factory）已无 `agent/provider/tool` 生产直连，Desktop app/tabs 已无 `provider` 生产直连；剩余集中在 Desktop session-store、CLI composition root 与其余专用渲染。

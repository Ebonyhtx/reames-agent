...
- [ ] 继续收口会话、装配与设置边界；稳定 `control.SessionInfo` / `ListSessions` / `ResumeSessionPath` 已承载 CLI、Bot、Serve 与 ACP 的列表/恢复路径，CLI branch-id/rename、跨进程 lease、延迟 cleanup、session-owned subagent 删除、Desktop trash/recovery GC 和设置延迟 rebuild 也已迁入 control。本轮从 CLI/Bot/Serve/ACP/Desktop 去除 10 条 `agent` 直连，连同此前 CLI resume 两条累计收缩 12 条；ACP replay 另已去除 `provider` 直连。同步 `RunTurn` 仍保留给拥有 turn 生命周期的 CLI/Bot/ACP，不冒充异步 command dispatch（本地进行中批次见 `audits/2026-07-11-m2-session-adapter-boundary.md`）。
...
- [ ] 按可验证的纵向路径收缩依赖 allowlist，不做一次性大搬家；本批已删除 ACP replay 对 `provider` 的一条直连基线。

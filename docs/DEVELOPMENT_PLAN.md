...
P4 不高于 P3 远端失败修复；若 candidate、CI 或 CodeQL 红灯，先修交付链再进入主题功能。
...
```text
P1（writer worktree）与 P2（offline Guard/Safe Mode）已关闭；P3 仓库内实现和本地 Windows smoke 已完成
→ 当前交付：完成 Root/Desktop/Frontend、race、六目标、contracts、clean-clone 全量，集中单次 push；
等待 CI、CodeQL 和三平台 Desktop candidate，失败则在同一批修复
→ P4（受控 Theme Pack）：先安全 manifest/原子内容寻址存储，再延迟 Gallery/可撤销预览，最后原创资产；

...
## 20. 2026-07-21 Reasonix / Codex 新批次

Reasonix `43993f5a..77fd1a47` 的 10 个提交已逐提交代码级分类：采用 workspace virtualizer 稳定 key、
ACP vendor steering、cc-switch Reames/Reasonix app flag 和显式 Goal；Remote SSH 继续 P11，WebView2 remote-image
proxy 与 current workspace diff 继续 P3/P9，Creation-mode z-index 对当前树不适用。Reames 不整项删除 automatic
Plan：它默认关闭且只能由用户显式开启，现又移除了普通提示隐式启动 Goal 的另一条隐藏模式切换。

Codex `678157ac..eceb3eea` 的 7 个提交也已代码级分类。legacy exec allow migration 的安全不变量已采用：
shell/interpreter/`git:*`/`rm:*`/package-script-wide 的可复用 allow 不能进入 runtime；精确命令仍可由用户保存。
zsh snapshot、unknown rollout history mode、inline visualization rewrite 和 TUI lifecycle/flex cache 分别进入
P9 headless/App-Server/streaming/performance 合同，不复制 Rust runtime。

Git smart-HTTP 当时不可用，full SHA、拓扑、逐提交文件和 patch 由 GitHub 官方 compare/commit API 取得并记录；
这不是 release-note 推断。锁已推进到 Reasonix `77fd1a47be89498febe16a940621f79cc84f0b92` 与 Codex
`eceb3eeaf3a68d732596fd8c0e8a6807f9166770`。完整证据见
`audits/2026-07-21-reasonix-43993f5-77fd1a4-codex-delta.md` 与
`upstreams/reviews/reasonix-generation-43993f5-77fd1a4.json`。

## 21. 2026-07-21 其余移动参考上游

Hermes、MiMo、Scream、Kimi 与先前网络失败的 Impeccable 已重新 fetch，并按 reviewed lock → 最新 `origin/main`
做代码级分类。Hermes 的 credential/transcript/sandbox、stream EOF 与 delivery-attempt 信号在 Reames 已有等价或更强
边界；MiMo 的路径规范化和空消息信号同样已有 rooted writer/control gate，GPT reasoning-only final 因违反可见答复合同
被拒绝；Scream 国内搜索 fallback、TUI preview/permission badge 进入 M7/P9 候选；Kimi unified transcript 和 live child
permission propagation 进入 P9 App-Server/权限 epoch；Impeccable source lock/event identity/current-findings 进入 P10/Hook
治理候选。Python/Electron/TS 第二 runtime、xAI OAuth、站点/品牌/设计规则与生成副本均不采用。

锁推进到 Hermes `456f18b19c4208115acbf0c6b226af49916b5480`、MiMo
`b7b2092a27def7b14df663fc7698165806185635`、Scream `ae8cd938c54abcb29a720af064b9e26d9b129d06`、
Kimi `c2d7bebd04106473bb4dbab2903756aa3f14a880` 与 Impeccable
`b906b41462c26c359e452040994685ce6d8e4008`。Impeccable 在首次精确接受时移动，脚本正确拒绝旧
`e6f3ce6d`；补审 template-extension/source-search 单一 owner 后才推进到新 SHA。完整证据见

...
- `README.md` 只描述当前入口：CLI、`serve`、`bot start`；不得再出现旧 `gateway start`。
...
3. 打开 Dependabot alerts 与 secret scanning；如果 GitHub 提供 CodeQL/code scanning，再恢复 CodeQL workflow。
...
仓库里仍保留部分从旧项目继承的脚本和 workers，例如旧 installer、release 辅助脚本、Hermes gateway 原型和 worker 目录。它们当前不在 README、发布文档或 CI 主路径中作为正式产品入口。

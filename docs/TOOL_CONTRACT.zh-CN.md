...
writer 子代理还会提供 `subagent_delivery_preview` 与 `subagent_delivery`：
前者只读投影隔离的 branch/worktree、文件、提交与测试证据，后者通过统一
Controller 边界执行 apply、merge、rollback 或 reject。
...
`subagent_delivery_preview` 与 `subagent_delivery` 在 economy 模式中仍保持
可用，使已有隔离交付可以继续被检查和收口。

`bash`、`read_file`、`grep`、文件写工具、后台 job 工具和 `todo_write` 等核心内置工具在 economy 模式下仍可用，见上方内置工具表。

## 均衡与交付工作模式

均衡是默认公开工作模式；旧会话中持久化的 `full` 会规范化为 `balanced`。均衡和交付暴露完全相同、顺序稳定的完整工具 schema。交付模式只改变 cache-stable system contract：要求复用现有 Todo、`complete_step`、project checks、checkpoint、权限和沙箱机制来明确验收标准、修复根因、运行聚焦验证、审阅 diff 并以证据收口。它不会建立第二套 Agent runtime，也不会额外授予工具。

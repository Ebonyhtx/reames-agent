...
| `cronjob` | false | Create, list, or delete persistent scheduled tasks. Supports interval (every 30m) and once (30m) schedules. Use for recurring automation like daily reports. |
| `list_jobs` | true | List all running and recently completed background jobs (bash and task) without blocking. Returns each job id, kind, label, status, and start time. Use to discover running jobs before reading output or killing them. |
| `bash_output` | true | Read new output from a background job started with bash(run_in_background=true) or task(run_in_background=true). Returns the output produced since the last bash_output call for that job, plus its status (running/done/failed/killed/interrupted). Interrupted means a recoverable task stopped with the prior process and was not replayed automatically. Does not block. |
...
| `web_search` | true | Search the web and return results with titles, URLs, and snippets. Use to find current information, documentation, or answers that may not be in your training data. Returns up to 10 results. |

...
Reames Agent already has Goal mode and AutoResearch instructions. When a goal looks
long-running, `activeGoalBlock` injects an AutoResearch protocol that asks the
model to create `.reames-agent/autoresearch/<task-id>/` and maintain files such as
...
- No network, publish, payment, credential, or destructive-operation bypass.
  Existing Reames Agent gates still apply.
...
2. If AutoResearch is on, the controller creates a task unless the goal contains
   an explicit `.reames-agent/autoresearch/<task-id>/` path.
...
- Explicit: a goal or prompt includes `.reames-agent/autoresearch/<task-id>/`.
...
`AutoResearchOpenTask` opens `.reames-agent/autoresearch/<task-id>/` in the
...
- Resume: starts or continues `/goal --research .reames-agent/autoresearch/<task-id>/`
...
- explicit `.reames-agent/autoresearch/<task-id>/` resumes existing state
...
No existing `.reames-agent/autoresearch` task should be deleted or rewritten without

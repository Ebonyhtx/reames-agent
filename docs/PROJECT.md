...
- M4 已完成会话运行态恢复、共享委派预算、writable effects、durable evidence、writer persistence 和后台 Task/compaction/memory 统一恢复六批：所有 Goal completion 统一通过 Todo/project checks，v2 sidecar 持久化 Goal/Plan/Todo 与最小项目检查引用；`task`、`read_only_task`、`parallel_tasks` 和 subagent skill 共享委派树预算，可写 child receipt/checkpoint 归并给祖先，previewable writer 在 checkpoint/runtime/in-flight 持久化失败时不执行。持久后台 task 现在在每个 Provider/tool/compaction 安全边界保存 transcript；job running metadata 失败则不启动，冷启动把未完成 job 恢复成带 `continue_from` 的 `interrupted` tombstone，显式续跑先要求重验 workspace，绝不自动重放副作用。统一测试还证明 compacted transcript 可续接，记忆检索保留 score/path/snippet、可关闭、可归档删除且动态命中不改变稳定 system prefix。该状态仍不等于 M4 完成：effects/预算和 child-only 验证仍是进程内状态，无静态目标的 shell 不具备逐文件门禁或 exactly-once，handle-relative 路径安全、durable child effect journal 与跨资源断电事务仍未关闭。

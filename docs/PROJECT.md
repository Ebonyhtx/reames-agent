...
- M3 已进入 Desktop 日用化：关闭态/次级界面与简中/繁中词典已按需拆包并受真实产物硬预算保护，模态隔离、Transcript 语义和严格 Windows UIA 可访问性 smoke 已交付。commit `16136c8`/`1d115ba` 的 CI `29248016550` 8/8、CodeQL `29248016498` 3/3；candidate `29248420722` 的 Linux/macOS installed 与 Windows native 通过，Windows interaction 完成持久化、19 请求、五类失败恢复和停止后仍未显示重启消息，accessibility 因前序失败未执行。当前 follow-up 在 controller ready 后强制刷新权威历史，live turn 才允许跳过；磁盘与 controller 用户轮次相同时，磁盘可见消息更多也胜出，并让 smoke 分别记录重启 tab/session、磁盘和 UIA 断点。本地新 production native、interaction/restart 和 strict accessibility 已全绿，仍需新的 installed candidate 复核。自动证据不等于 NVDA/Narrator 实际听感或 Windows High Contrast 人工验证。

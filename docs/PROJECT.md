...
- M5 插件生命周期信任机制已进入跨宿主验收：原生 schema v1 要求语义版本和精确权限，复制安装发布内容寻址不可变 generation，新安装默认禁用，启用绑定 digest/权限；update/rollback/uninstall 使用 preview/planId/apply，状态写入有跨进程锁，受管路径使用 `os.Root`，并有 tamper、link 漂移、发布/状态/回滚/清理失败注入及 CLI 完整生命周期测试。Desktop 自动化、真实 Chromium、源码 production Wails 和 commit `e9de895` 的已安装 Windows candidate 已分层证明 stale plan 拒绝、默认禁用、精确授权、generation 更新/回滚、doctor、移除和无默认状态泄漏；该提交的 CI `29378573077` 8/8、CodeQL `29378573116` 3/3、candidate `29378899444` 三平台全绿。插件 MCP owner 固定在 controller 实际加载的配置并随同名用户接管更新，generation 变化或禁用会以 controller runtime-mutation reservation 和 Desktop work-start gate 原子阻止检查后新 turn/rotation 起跑，串行化同步 rebuild、取消旧状态的迟到 startup build，并在所有 live/detached controller 撤销旧 MCP/Hook、让旧 Skill 入口 fail closed 到重建/新会话；MCP connect/owner 更新同样串行，同名用户接管不被误撤销。`install_source` 精确计划已接入 Controller、Desktop、CLI、Bot、Serve/event wire 和 ACP 的 fresh-human 审批，禁止自动策略代答，让不支持及 headless 宿主在预览前 fail closed，并将 MCP 执行字段结构化脱敏后展示。已安装包拥有的 Hook/MCP 使用 core-env + 显式变量、独立 state/tmp、严格 OS sandbox、敏感 read barrier 和 fail-closed backend；manifest 环境原始键值不进入 wrapper argv，隐藏 helper 只在隔离边界内恢复并清除编码 child env，缺失 payload 或 dispatch 时拒绝启动。Hook 撤销会取消运行中进程树并阻断 late start。Windows 原生 helper 已验证 child env/cwd 与越界写拒绝，真实 `obra/superpowers@d72560e462a74e10d161b7f993d5fc3282bfa1e2` 完成 unsigned 固定 revision 安装、精确授权和 sandbox SessionStart E2E。commit `13016c6` 加入无默认 endpoint/TOFU 的官方 TUF client：用户级 endpoint 与带外 bootstrap root 不受项目 TOML/`.env` 控制，持久 metadata 验证轮换/过期/rollback/freeze/mix-and-match，严格索引和 apply 绑定 full commit、树摘要、版本、权限与 provenance assertion，CLI/Desktop 可发现并展示落盘证据；其最终 clean clone、普通 CI `29487948296` 8/8 与 CodeQL `29487948316` 3/3 全绿。当前本地批次进一步增加只读生产策略审计：从显式带外 root 重放连续旧/新双阈值轮换，强制角色密钥隔离和到期窗口，并验证完整 index/attestation 字节，同时在成功报告中保留外部仪式/HSM/endpoint/monitor/DSSE-SLSA 边界。该事实仍不等于 M5 完成：直接 GitHub 未签名，尚无已运营公开 registry、真实生产密钥仪式/轮换/compromise drill 或 DSSE/SLSA policy verifier，跨平台统一硬 CPU/RSS 配额不在当前声明内；任何后续候选仍须通过干净 clone 与最新远端 CI/CodeQL。

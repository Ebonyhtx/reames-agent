...
- M5 插件生命周期信任机制已进入跨宿主验收：原生 schema v1 要求语义版本和精确权限，复制安装发布内容寻址不可变 generation，新安装默认禁用，启用绑定 digest/权限；update/rollback/uninstall 使用 preview/planId/apply，状态写入有跨进程锁，受管路径使用 `os.Root`，并有 tamper、link 漂移、发布/状态/回滚/清理失败注入及 CLI 完整生命周期测试。Desktop 自动化、真实 Chromium、源码 production Wails 和 commit `e9de895` 的已安装 Windows candidate 已分层证明 stale plan 拒绝、默认禁用、精确授权、generation 更新/回滚、doctor、移除和无默认状态泄漏；该提交的 CI `29378573077` 8/8、CodeQL `29378573116` 3/3、candidate `29378899444` 三平台全绿。插件 MCP owner 固定在 controller 实际加载的配置并随同名用户接管更新，generation 变化或禁用会以 controller runtime-mutation reservation 和 Desktop work-start gate 原子阻止检查后新 turn/rotation 起跑，串行化同步 rebuild、取消旧状态的迟到 startup build，并在所有 live/detached controller 撤销旧 MCP/Hook、让旧 Skill 入口 fail closed 到重建/新会话；MCP connect/owner 更新同样串行，同名用户接管不被误撤销。本批又把 `install_source` 精确计划接入 Controller、Desktop、CLI、Bot、Serve/event wire 和 ACP 的 fresh-human 审批，禁止自动策略代答，让不支持及 headless 宿主在预览前 fail closed，并将 MCP 执行字段结构化脱敏后展示；完整本地 Go/race、Desktop、前端、合同、真实 Chromium、upstream 和六目标交叉编译门禁均已通过，提交与远端状态须以当前 `git log`/GitHub Actions 单独核验。该事实不等于 M5 完成：GitHub 仍未签名、无已运营默认 registry，插件进程 sandbox、真实第三方插件和干净 clone 仍待收口，关闭时最新提交必须远端全绿。

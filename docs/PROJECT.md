...
- Reasonix `dae65e25` 的会话可靠性机制已按 Reames 架构吸收：非成功 HTTP body 有硬读取边界；verified save 的 snapshot no-op 仍以 transcript/event-log 文件戳和 ledger revision/digest 做磁盘 fencing；损坏 event-log 尾部只有在取证 sidecar 成功落盘后才截断；Desktop navigation 以 monotonic epoch 保证最后点击优先。Reasonix 删除 Memory Compiler 的产品取舍未直接跟随；Reames 保留自身合同并在 Safe Mode 禁用，后续变更必须由本项目的 provider-visible 收益、loop 风险和 cache benchmark 决定。下一内部方向是安装态 Recovery Center/发布候选证据与 M6 远程运维闭环；外部风险仍是公开 registry 运营、干净云节点 logout/reboot、真实 IM 回环和生产签名链。

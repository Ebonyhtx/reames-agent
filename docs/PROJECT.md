...
- M4 已完成会话运行态恢复、共享委派预算和 writable effects 三批：所有 Goal completion 统一通过 Todo/project checks，v2 sidecar 在 turn/history rewrite/transition 边界持久化 Goal/Plan/Todo、continuation 计数和 transcript digest；Resume/Branch/Switch/Fork/Rewind 整体恢复，Checkpoint divergence/legacy/负边界/无效 runtime fail closed，durable truncate tombstone 与 path/case/hard-link alias 回滚已覆盖。`task`、`read_only_task`、`parallel_tasks` 和 subagent skill 按一棵委派树共享活跃 Provider round、聚合 step/token/time 与 root cancellation；可写 child 的结构化 read/write/command receipt、父 tool-call provenance 和 previewed pre-edit checkpoint 会归并给祖先，partial failure/cancel 形成 mutation boundary，父 checkpoint 可回退 child 文件。迟到的后台 receipt/checkpoint 由 evidence generation 和 turn-scoped callback 拒绝写入新 turn。该状态不等于 M4 完成：effects/预算仍是进程内状态，durable evidence、writer fail-closed、handle-relative 路径安全、跨资源断电事务和后台 Task crash-resume 仍未关闭。

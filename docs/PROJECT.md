...
- M3 已进入 Desktop 日用化：关闭态/次级界面与简中/繁中词典已按需拆包并受真实产物硬预算保护，模态隔离、Transcript 语义和严格 Windows UIA 可访问性 smoke 已交付。commit `bb13da3` 的 CI `29256586177` 8/8、CodeQL `29256588974` 3/3；candidate `29257178248` 的 Linux/macOS installed 与 Windows native 通过。Windows interaction 完成 19 请求、五类失败恢复和停止，重启 tab/workspace/session 与磁盘 user/assistant 均正确，UIA 也有 user，但直接检查首轮 assistant 仍缺失，accessibility 因前序失败未执行。这证明 pre-ready history replacement 是有效竞态硬化但不是已确认的 installed 失败根因；当前证据更符合首轮消息离屏时 `.msg` 的 `content-visibility` 与 UIA 可见性约束。smoke 现通过可访问问题导航定位首轮后再验证 user/assistant；本地 production 已复现跳转前双消息存在但均 offscreen、跳转后均 onscreen，并完成 19 请求 interaction/restart，仍需新的 installed candidate 复核并实际执行 strict accessibility。自动证据不等于 NVDA/Narrator 实际听感或 Windows High Contrast 人工验证。

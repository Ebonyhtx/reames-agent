...
- OpenAI Codex 与 Claude Code 是二级战略代码上游：分别跟进 GPT/OpenAI 与 Claude/Anthropic 的原生
  ...
  Browser/CDP 等代码级产品能力；仍按 Reames Go/Wails 架构重构，不作为可直接拼接的 runtime。
...
- M6 的无界面配置与 credential-free 运维预检已形成纵向链路：`gateway setup` 支持飞书/Lark、QQ、微信和 Telegram，secret 只引用大写环境变量名，访问控制 fail closed，dry-run 零落盘，更新原子、幂等并保留其他连接、route、access 和 session mappings；同一隔离 home 的真实 CLI 二进制覆盖 setup → doctor → service-plan、localhost Provider 一次性 `run`/会话落盘、反馈脱敏/去重/维护草稿。WSL2 真实 systemd user manager 进一步通过带空格 binary/home/workspace 的 install、同名重装生效、status、restart、stop/start、journal、loopback webhook readiness 与 uninstall `LoadState=not-found`，但 `Linger=no`，不替代 logout/reboot、干净云节点、真实 Provider 或真实 IM 回环证据。
...
- M6 Gateway core 已吸收 Hermes durable channel recovery 信号：CLI/Desktop 共用 Reames home 下的 0600 原子入站账本，消息先 claim 再进入访问控制/排队/Agent；重复消息跨重启抑制，遗留 processing 冷启动转为可重试。游标按远端频道和单调序列只推进连续成功前缀；collect/debounce 合并、queue-cap summarize/drop、interrupt 与 `/stop|/new|/reset|/use|/attach` 均保留或显式结算全部 constituent claims，不会只提交首条消息。最终 render 发送失败、账本损坏/超限/身份不一致/写失败均不推进；启动补扫受全局 200 条上限约束，状态与指标只显示计数。Telegram 已成为正式 long-poll Adapter：`getMe` 启动验证、请求 deadline、指数退避、可取消 Stop、`update_id` durable identity、原生 `message_id` reply 和最终 ledger commit 后 offset 推进均有 localhost 故障注入。飞书/QQ/微信仍缺真实历史分页 `RecoveryAdapter`，Telegram 也尚无独立历史 API 补扫；完全离线补消息、真实 IM 掉线回环和 outbound final-response obligation（不重跑模型的答复恢复）仍是 adapter/external-blocked 或下一 M6 durability 缺口。详见 `audits/2026-07-19-m6-durable-channel-recovery.md` 与 `audits/2026-07-19-m6-telegram-durable-polling.md`。
...
- 最新一级上游已代码级审至 Reasonix `8bb0e549`：在上一批可靠性采用基础上，本批修正 LongCat-2.0 的 1,048,576 context window 与窄迁移，回移 Linux WebKit/JavaScriptCore 启动信号修复，并完成会话图片/PDF 分页、安全资源隔离、原子多文件导出与可见错误。最新 settings-refresh Theme Pack 修复由 Reames 统一 `themePackRuntime` 既有边界等价覆盖并补显式回归；pane opacity 因 Theme Pack v1 schema 已关闭而延后，Remote SSH 作为 P11 新安全面不整套复制。Codex 仍为 `0fb559f0`，Claude 为 `015170d3`；Hermes/Kimi/Scream 最新增量只做机制分类。完整结论见 `audits/2026-07-19-reasonix-a46fc6f-65fcd46.md` 与 `audits/2026-07-19-reasonix-65fcd46-8bb0e54.md`。

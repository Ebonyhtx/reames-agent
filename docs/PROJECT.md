...
- M5 插件生命周期信任机制已进入原生本地验收：原生 schema v1 要求语义版本和精确权限，复制安装发布内容寻址不可变 generation，新安装默认禁用，启用绑定 digest/权限；update/rollback/uninstall 使用 preview/planId/apply，状态写入有跨进程锁，受管路径使用 `os.Root`，并有 tamper、link 漂移、发布/状态/回滚/清理失败注入及 CLI 完整生命周期测试。Desktop 自动化和真实 Chromium 已覆盖安装、授权、更新、回滚、移除的两阶段 UX；源码 production Wails 进一步用真实 Go 后端和隔离落盘状态证明 stale plan 拒绝、默认禁用、精确授权、generation 更新/回滚、doctor、移除和无默认状态泄漏。插件 MCP owner 固定在 controller 实际加载的配置并随同名用户接管更新，generation 变化或禁用会以 controller runtime-mutation reservation 和 Desktop work-start gate 原子阻止检查后新 turn/rotation 起跑，串行化同步 rebuild、取消旧状态的迟到 startup build，并在所有 live/detached controller 撤销旧 MCP/Hook、让旧 Skill 入口 fail closed 到重建/新会话；MCP connect/owner 更新同样串行，同名用户接管不被误撤销。该事实不等于 M5 完成：GitHub 仍未签名、无已运营默认 registry、插件进程 sandbox/宿主统一审批、真实第三方插件和远端 installed candidate 证据仍待收口。

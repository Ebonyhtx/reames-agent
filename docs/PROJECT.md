...
- M3 已进入 Desktop 日用化：关闭态/次级界面与简中/繁中词典已按需拆包并受真实产物硬预算保护，模态隔离、Transcript 语义和严格 Windows UIA 可访问性 smoke 已交付。commit `977050c` 的 CI `29252169252` 8/8、CodeQL `29252169424` 3/3；candidate `29252663874` 的 Linux/macOS installed 与 Windows native 通过。Windows interaction 完成 19 请求、五类失败恢复和停止，重启 tab/workspace/session 与磁盘 user/assistant 均正确，UIA 也有 user，但 assistant 缺失，accessibility 因前序失败未执行。新字段把根因收敛到 ready 事件错误复用尚未完成的 pre-ready history 请求；当前 follow-up 将该预载标为可取代，让 ready 后权威请求启动新 sequence，并丢弃迟到的部分结果。本地新 production native、19 请求 interaction/restart 和 strict accessibility 已全绿，仍需新的 installed candidate 复核。自动证据不等于 NVDA/Narrator 实际听感或 Windows High Contrast 人工验证。

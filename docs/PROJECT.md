...
- M3 已进入 Desktop 日用化：关闭态/次级界面与简中/繁中词典已按需拆包并受真实产物硬预算保护。可访问性后续已由 commit `827e0b4` 交付：真正模态层具备 `inert`/`aria-hidden` 隔离、嵌套顶层和稳定 dialog ID，Transcript 使用非实时 `role=log` 与独立最终答复状态区，Windows UIA 具备属性检查和严格 InvokePattern smoke。M3+M6 commit `c2dc1a3` 的普通 CI `29241651878` 8/8、CodeQL `29241651981` 3/3；candidate `29242006740` 的 Linux/macOS installed 与 Windows native 通过，但 interaction 仍于持久化、19 请求、五类失败恢复和停止完成后未显示重启消息，accessibility 因前序失败未执行。当前 follow-up 不再要求首份 tab metadata 带 session path，并让空 controller transcript 回退 0 B checkpoint 对应的 canonical event log；本地 production native、interaction/restart 与 strict accessibility 已全绿，仍需新 installed candidate 复核。自动证据不等于 NVDA/Narrator 实际听感或 Windows High Contrast 人工验证。

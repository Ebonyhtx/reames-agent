...
- M4 已完成会话运行态恢复、共享委派预算、writable effects、durable evidence、writer persistence、后台 Task/compaction/memory、rooted writer/checkpoint 与 durable child journal 八批：所有 Goal completion 统一通过 Todo/project checks，v2 sidecar 持久化 Goal/Plan/Todo、最小 root 项目检查引用和 child journal cursor；`task`、`read_only_task`、`parallel_tasks` 和 subagent skill 共享委派树预算，可写 child receipt/checkpoint 归并给祖先。所有 previewable built-in writer 与 checkpoint restore 已使用 `os.Root` + 相对路径完成 resolve-beneath read/stat/write/rename/remove，multi-file preview 会在执行前全部快照；顶层持久 `task`/writer `run_skill` 在 child writer 执行前持久化 mutation intent，恢复时以 parent session/tool-call、workspace、journal ID/sequence 和 cursor 防跨分支与重复失效。持久 subagent 在 Provider/tool/compaction 边界保存 transcript，冷启动以 `interrupted` + `continue_from` 恢复且不自动重放副作用。该状态仍不等于 M4 完成：完整 evidence/预算仍非跨进程账本，child-only bash 不是 durable root proof，shell/MCP/external API 不具备逐文件门禁或 exactly-once；transcript/runtime/checkpoint/workspace 跨资源断电事务、`AtomicWriteFile` Windows degraded fallback、ACL/xattr/硬链接身份仍未关闭。

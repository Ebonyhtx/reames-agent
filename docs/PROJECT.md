...
- 当前最大风险不是“缺功能”，而是剩余 Desktop session-store、CLI composition root 与其余专用渲染 control 边界、插件供应链、远程入口加固和真实 IM 回环尚未完全闭环；版本化 command/event/display DTO、prompt metadata、主要会话持久化/复制、ACP 装配、设置及 Desktop history 路径已收口。

...
- M3 已进入 Desktop 日用化：关闭态/次级界面与简中/繁中词典已按需拆包并受真实产物硬预算保护。可访问性后续已由 commit `827e0b4` 交付：真正模态层具备 `inert`/`aria-hidden` 隔离、嵌套顶层和稳定 dialog ID，Transcript 使用非实时 `role=log` 与独立最终答复状态区，Windows UIA 具备属性检查和严格 InvokePattern smoke。commit `96c0fe2` 的普通 CI `29244107302` 8/8、CodeQL `29244107307` 3/3；candidate `29244551372` 的 Linux/macOS installed 与 Windows native 通过，Windows interaction 也完成持久化、19 请求、五类失败恢复和停止，但重启消息仍不可见，accessibility 因前序失败未执行。本批继续关闭两个此前未覆盖的“假完整”门槛：controller 只有部分轮次时改用轮次更多的 canonical event log，前端启动 notice 不再冒充可复用 transcript；本地 production native、interaction/restart 和 strict accessibility 已全绿，仍需新的 installed candidate 复核。自动证据不等于 NVDA/Narrator 实际听感或 Windows High Contrast 人工验证。

...
- M3 已进入 Desktop 日用化：关闭态/次级界面与简中/繁中词典已按需拆包并受真实产物硬预算保护。可访问性后续已由 commit `827e0b4` 交付：真正模态层具备 `inert`/`aria-hidden` 隔离、嵌套顶层和稳定 dialog ID，Transcript 使用非实时 `role=log` 与独立最终答复状态区，Windows UIA 具备属性检查和严格 InvokePattern smoke；普通 CI `29229513429` 8/8、CodeQL `29229513359` 3/3。Desktop candidate `29229871453` 的 Linux/macOS 安装候选和 Windows native 启动通过，但 Windows interaction 在两次 attempt 中都于 19 请求、五类失败恢复和停止完成后，因恢复 controller 超过 30 秒而未显示已持久化消息，accessibility step 因前序失败未执行。当前批次把持久 transcript 可见性与 controller 可发送状态解耦：`ready=false` 时先从 pinned session 原子事件账本预载历史，仍保持发送锁定，controller ready 后再补 ancillary；完整前端、production Wails native/accessibility/interaction 已本地通过，仍需新 commit 的安装器 candidate 复核。自动证据不等于 NVDA/Narrator 实际听感或 Windows High Contrast 人工验证。
- M6 的无界面配置缺口已在当前批次实现：`gateway setup` 支持飞书/Lark、QQ、微信，secret 只引用大写环境变量名，访问控制 fail closed，dry-run 零落盘，更新原子、幂等并保留其他连接、route、access 和 session mappings；确定性 `setup -> doctor -> service dry-run` 已通过，干净云节点与真实 IM 回环仍待外部证据。

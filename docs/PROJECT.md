...
- 2026-07-17 Reasonix 再同步已采用 MCP schema 严格验证/坏工具隔离、保存凭据的子进程与文件读取保护、credential-free schema cache identity、Anthropic 累计 usage 和 MiMo 专用 schema dialect；同时确认两个新的内部高优先级缺口：用户 MCP 的 `trusted_read_only_tools` 尚未绑定 server/tool identity receipt，可写 child 虽有 durable effect journal 但尚无 workspace lease/独立 worktree。当前最大风险不是“缺更多 UI 功能”，而是这些本地信任/并发 writer 边界，以及插件供应链、远程入口与云节点运维、真实 IM 回环及生产签名链尚未完全闭环。统一 control 边界和 Desktop 日用化门槛已关闭并有远端 CI/CodeQL/candidate 证据，transport 对 `agent/provider/tool` 的生产直连为零，版本化 command/event/display DTO、prompt metadata、会话持久化/复制、Desktop session-store、ACP/CLI 装配和终端渲染路径已收口。

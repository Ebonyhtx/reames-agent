...
- 本批进一步关闭 M6 渠道决策/取消/连接状态仓库内合同：Controller 对未知或过期 approval/ask 返回 `not_found`；远端 decision token 带随机 epoch 且一次性消费，Gateway 重启不复用。四渠道 `/stop` 取消 fixture 验证 active turn、queued claims、成功连续 checkpoint、ACK 失败 retryable 和每条 settlement 恰好一次，避免命令内部与外层重复 ACK。Feishu、QQ、微信、Telegram 统一投影 `connecting/running/reconnecting/closed`，status/metrics 只给固定 reason 与 reconnect count；CLI watchdog 在恢复后继续 heartbeat，QQ Resume 仅证明在线 session 恢复。详见 `audits/2026-07-21-m6-channel-decisions-cancellation-reconnect.md`。

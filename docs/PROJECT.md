...
- 2026-07-17 Reasonix 再同步已采用 MCP schema/凭据/Provider 加固，并关闭 identity-bound MCP trust P0 与 writer worktree P1。可写 task/Skill/Subagent 现在使用独立 `reames/subagent-*` branch/worktree、workspace/ref 跨进程锁和 workspace-bound tool registry；只读委派保持轻量共享，非 Git writer fail closed。取消/崩溃、lost/orphaned、回收站恢复和永久 purge 有明确生命周期；父会话通过统一 Controller preview/apply/merge/rollback/reject，CLI/Serve/Desktop 只投影同一 DTO。source mutation 前持久化 acceptance intent，无法排除人工 drift 的 post-apply 崩溃转为 `acceptance_interrupted`，不自动覆盖。内部下一优先级转为复用现有 updater/recovery transaction 的 offline Guard/Safe Mode 评估；外部风险仍是公开 registry 运营、远程入口与云节点运维、真实 IM 回环和生产签名链。

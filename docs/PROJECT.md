...
- M4 已进入进行中并完成第一批会话运行态恢复：所有 Goal completion 统一通过 Todo/project checks，重复声明和被 Hook 拦截的 synthetic self-check 不再绕过证据门；v2 sidecar 在 turn/history rewrite/transition 边界持久化 Goal/Plan/Todo、continuation 计数和 transcript digest，append-only transcript 从 durable Todo 基础重放后缀。Resume/Branch/Switch/Fork/Rewind 整体恢复；Checkpoint prefix digest 拒绝同长度 divergence、legacy 无摘要、负边界和无效 runtime，durable truncate tombstone 阻止删除失败后重启复活，路径/大小写/现存硬链接 alias 共享 earliest bytes，文件中途失败连同可能部分写入的目标一起回滚。该批不等于 M4 完成：共享子代理预算、writable 归并、durable evidence、writer fail-closed、handle-relative 路径安全、跨资源断电事务和后台 Task crash-resume 仍未关闭。

...
- 阅读报告也不会自动视为接受；
- 只有显式 `--accept <id>` 并提交 lock 文件，才表示该版本已审查。

## 3. 自动判定

| Decision | 含义 |
|---|---|
| `up-to-date` | 官方分支与已审查版本一致 |
...
报告包含稳定的 state fingerprint。生成时间不会改变指纹；只有 reviewed/latest/decision/error 状态变化才会产生新指纹。
...
```powershell
python scripts/check_upstreams.py --accept reasonix --out-dir artifacts/upstream-watch
```

可重复 `--accept`：

```powershell
python scripts/check_upstreams.py --accept reasonix --accept codex
```

确认 `docs/upstreams/upstreams.lock.json` 的 diff 后提交。不要使用 `--accept-all`，除非所有上游都确实逐一审查完成。

`--update-lock` 只作为旧命令兼容入口，等价于 `--accept-all`。

## 7. Reasonix 升级审查顺序

Reasonix 是 primary upstream，按以下顺序审查：

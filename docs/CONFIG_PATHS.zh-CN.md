...
可以设置 `REASONIX_HOME` 覆盖 Reames Agent home，主要用于测试、CI 或便携安装。普通用户通常不需要设置。
...
写到 `REASONIX_STATE_HOME/.env`，Reames Agent 会在 `<Reames Agent home>/.env` 缺少对应 key 时非破坏性导入。
...
`<state root>` 默认等于 `<Reames Agent home>`；只有设置 `REASONIX_STATE_HOME`
时才会不同。
...
项目 `.env` 仍会作为当前 workspace 范围内的非 provider 变量展开来源，例如 MCP/plugin 的 env、headers、URL、command 和 args 中的 `${VAR}`；这些值不会写入进程环境，`REASONIX_HOME`、`REASONIX_STATE_HOME`、`XDG_CONFIG_HOME` 等 Reames Agent 控制变量也会被忽略。

缓存仍放在系统缓存目录，例如 macOS 的 `~/Library/Caches/reames-agent`、
Linux 的 `$XDG_CACHE_HOME/reames-agent` 或 `~/.cache/reames-agent`、Windows 的
`%LOCALAPPDATA%\reames-agent\cache`。可以设置 `REASONIX_CACHE_HOME` 覆盖缓存根目录。

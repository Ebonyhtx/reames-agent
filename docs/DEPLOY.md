...
reames-agent gateway install --start-now --watchdog-sec 60s --home "$REAMES_AGENT_HOME"  # Linux systemd 可选 watchdog
...
`Type=notify`、`NotifyAccess=main` 和 `WatchdogSec=`。Gateway 只有在共享 recovery preflight 完成并至少
一个 adapter 启动后才发送 `READY=1`；至少一个 adapter 为 `running`/`degraded` 时才发送
`WATCHDOG=1`，全部进入 `closed`/`error` 后停止心跳；SIGINT/SIGTERM 会发送 `STOPPING=1` 并执行
30 秒有界停止。这个状态投影不能证明真实 IM 远端仍可达，生产节点仍需真实渠道回环和 watchdog
kill/restart 演练。
...
reames-agent gateway install --dry-run --home "$REAMES_AGENT_HOME" --channels feishu --dir /srv/project
reames-agent gateway install --start-now --watchdog-sec 60s --home "$REAMES_AGENT_HOME" --channels feishu --dir /srv/project

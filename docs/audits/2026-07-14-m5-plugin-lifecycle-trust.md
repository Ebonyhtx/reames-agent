...
12. 已安装包拥有的 Hook/MCP 现在携带不可伪造的 owner/root/state/home/workspace policy，
    wrapper 只继承 core environment，child 只接收显式 env；严格 OS sandbox 将写入限制在
    workspace/state/tmp，并阻断已知敏感目录/凭据文件读取。backend 不可用 fail closed；
    Hook timeout、disable 和 late-start 竞态会回收/拒绝进程树。真实
    `obra/superpowers@d72560e462a74e10d161b7f993d5fc3282bfa1e2` 已完成 Windows 原生
    sandbox 安装、精确授权和 SessionStart E2E。完整证据见
    `2026-07-15-m5-plugin-process-isolation.md`。
...
- GitHub 来源仍是 unsigned HTTPS；没有运营中的默认 registry、Reames 签名、
  provenance 或密钥轮换。
- package-owned Hook/MCP 已进入强制 OS sandbox，运行中 Hook/MCP 可随 owner 撤销回收；
  但用户手工 Hook/MCP 与 LSP 不自动隔离，package process 当前允许网络，也没有跨平台
  统一硬 CPU/RSS 配额。Skill fail-closed 仍会暂时暂停该 controller 的全部 Skill 入口，
  而不是原地热替换单个插件。
...
- 真实第三方插件 E2E 已由固定 revision superpowers Windows sandbox 链补齐；最终候选的
  干净 clone 仍必须完成，关闭 M5 时最新提交的远端 CI/CodeQL 必须全绿。
  在这些边界关闭前 M5 保持“进行中”。

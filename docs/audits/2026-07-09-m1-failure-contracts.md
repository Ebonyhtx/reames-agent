...
```powershell
go test ./internal/control -run "TestProviderAuthErrorEmitsTurnDoneAndClearsRuntimeStatus|TestApprovalTimeoutBlocksWriteAndClearsPendingPrompt" -count=1 -timeout 120s
```
...
- 工具运行超时在 Controller/Desktop 事件层的展示合同。

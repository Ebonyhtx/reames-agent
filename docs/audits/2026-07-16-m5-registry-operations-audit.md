...
远端 run `29487948296`/`29487948316` 虽全绿，但 GitHub 对 checkout、setup-go、
...
fail closed，只有记录过的 Node 24 tag commit 可作为 immutable pin。该迁移仍需本批最终远端 CI 证明，不以 actionlint 或本地 YAML
检查替代真实 runner 结果。
...
目录。远端 CI/CodeQL 仍须在本批集中 push 后取得，不由上述本地证据替代。

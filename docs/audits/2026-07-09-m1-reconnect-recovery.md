...
- Desktop 后端可以从上次保存的 `desktop-tabs.json` 恢复会话：classic 布局恢复多 tab、active tab、project workspace、pinned session path 和历史消息；默认 workbench 单表面布局只恢复 active tab，但同样保留 active project 的 workspace、session path 和 history。
...
- 新增 `TestRestoreOrBuildTabsRestoresSavedProjectSessionWorkspaceAndActiveTab`，模拟 classic 布局上次退出保存 Global + Project tabs 后，新 `App.restoreOrBuildTabs()` 恢复顺序、active tab、workspace root、session path 和 history。
- 新增 `TestRestoreOrBuildTabsWorkbenchRestoresOnlyActiveProjectSession`，锁定默认 workbench 单表面布局的恢复语义：只保留 active tab，但 active project 的 workspace、session path 和 history 不能丢。
...
Push-Location desktop; go test . -run TestRestoreOrBuildTabsWorkbenchRestoresOnlyActiveProjectSession -count=1 -timeout 180s; Pop-Location
```

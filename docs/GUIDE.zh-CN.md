...
避免旧任务为新回答提供伪证，此时必须从 job 结果与磁盘重新核验。该 bridge 只是进程内状态，
不是 crash-resumable 或 durable evidence。`bash` 无法静态 preview 目标文件，因此 child `bash`
与 root `bash` 一样没有自动逐文件 checkpoint。

...
- [App-Server 集成](./APP_SERVER.zh-CN.md)
...
优先级：**flag > `./reames-agent.toml` > 用户配置文件 > 内置默认值**。从
**Reames Agent v1.8.1** 开始，用户配置位于 macOS/Linux 的
`~/.reames-agent/config.toml`，Windows 为 `%AppData%\reames-agent\config.toml`；迁移和相关数据路径见
[配置路径](./CONFIG_PATHS.zh-CN.md)。标注为“仅用户/全局”的字段（包括 agent 轮数上限）不会被 `./reames-agent.toml` 覆盖。
Provider 通过 `api_key_env` 命名密钥，真实密钥值保存在 CLI 与桌面端共用的
Reames Agent 全局 `<Reames Agent home>/.env`。项目 `.env`、home `.env`、继承的 shell 环境变量、旧 credentials 和系统 keyring 都不再作为 provider key 的运行时 fallback；旧凭据只作为迁移来源读取。项目 `.env` 仍会作为当前 workspace 范围内的 MCP/plugin 非 provider `${VAR}` 展开来源，但不会导入 provider key 或 Reames Agent 控制变量。全局 `config.toml` 和 `.env` 的完整结构见
[配置路径](./CONFIG_PATHS.zh-CN.md)。
...
插件包可通过 `hooks/session-start-codex` 或插件根目录 `CLAUDE.md` 提供该启动上下文；Claude 风格 `.claude/settings.json` command hooks 也会按同名事件映射到 Reames Agent hooks。

## 工作模式

工作模式与普通/计划/目标协作方式、工具审批模式彼此独立。新会话默认使用 `balanced`：

```bash
reames-agent --profile economy
reames-agent run --profile delivery "完成并交付这个改动"
reames-agent serve --profile balanced
reames-agent acp --profile delivery
```

- `economy`：可选工具面按需连接，降低固定 schema 成本。
- `balanced`：暴露完整稳定工具面，是默认模式。
- `delivery`：不改变工具和安全门禁，在稳定 system contract 中额外要求验收标准、相关项目检查、证据、diff 审查和持续收尾。

交互式 TUI 可用 `/work-mode economy|balanced|delivery` 切换；恢复会话时，未显式传
`--profile` 会继承保存的模式。Desktop 输入框提供相同三项选择；Bot/Gateway 明确固定为
`balanced`。完整组合关系见 [协作方式与工作模式](./COLLABORATION_MODES.zh-CN.md)。

```toml
default_model = "deepseek-flash"   # 执行器；设 [agent].planner_model 可加规划器
# language    = "zh"               # 界面语言；为空则按 $LANG / $REAMES_AGENT_LANG 自动检测
...
# subagent_max_concurrency = 3         # 仅用户/全局；每棵委派树的活跃 Provider round
# subagent_max_steps = 100             # 仅用户/全局；每棵树聚合 Provider round
# subagent_max_tokens = 0              # 仅用户/全局；聚合上报 token；0 表示不限
# subagent_max_duration_seconds = 0    # 仅用户/全局；整棵树墙钟预算；0 表示不限
...
Reames Agent 也可以提示你确认是否信任它为只读；选择持久信任会写入同一个
`[agent].plan_mode_read_only_commands` 配置。Auto/YOLO 审批不会替用户回答这个信任提示。

### 环境变量

多数日常设置应写在 `config.toml` 或前文提到的 Reames Agent 全局 `.env` 中。下面这些变量是进程级高级开关；
需要在启动 Reames Agent 之前设置。项目 `.env` 不是 Reames Agent 控制变量的运行时来源。

`REAMES_AGENT_MEMORY_COMPILER_LLM_CLASSIFICATION=true` 会为 Memory v5 启用可选的 LLM 任务/聊天分类器。
默认关闭，此时 Reames Agent 使用本地 heuristic classifier，不会产生额外 provider 调用。开启后，分类缓存未命中时，
Reames Agent 可能先通过已配置 provider 发送一个很小的分类请求，再决定用户输入是任务还是普通对话；这会增加少量延迟、
...
`reames-agent serve` 会用同一个本地 Reames Agent 引擎启动浏览器 UI。适合不安装桌面端但想用可视化界面、
...
```bash
reames-agent serve --auth token
reames-agent serve --addr 0.0.0.0:8787 --auth token
...
`reames-agent app-server` 通过有界 stdio JSONL 协议把本地 Controller 暴露给编辑器和自动化客户端。
这是首批、范围明确的 Codex-class 兼容面，不代表完整 Codex App-Server parity。接入前请阅读
[App-Server 指南与方法矩阵](./APP_SERVER.zh-CN.md)。
...
预设路径通常只需要填写服务商 API Key：真实 key 会写入 Reames Agent home `.env`，
...
开启 **完整 URL**。开启后 Reames Agent 会直接使用该地址，不再追加 `/chat/completions`。
输入框下方的预览就是最终请求地址。
...
| 上下文窗口 | 该 provider 可保留的最大上下文 token 数。`0` 表示使用模型服务默认值。 | 模型实际上下文大小和 Reames Agent 默认值或内置元数据不一致时填写。 |

模型能力模式选项：

| 选项 | 作用 |
| --- | --- |
| 自动识别（推荐） | Reames Agent 根据模型能力元数据和端点自动选择请求格式。 |
...
| 选项 | 作用 |
| --- | --- |
| Auto（使用服务默认） | 不写 provider 级 `thinking` 覆盖，让 Reames Agent 使用 provider/model 默认行为。 |
...
桌面端快捷键在 **设置 → 快捷键** 中管理。选择一行后按下新的组合键，Reames Agent 会为桌面端保存该绑定。
...
| 终端原生选择 | 复制 transcript 文本 | Reames Agent 默认不启用鼠标报告，因此终端自己的选择/复制仍可使用。 |
...
`reames-agent` 会在 writer 调用前征求同意（普通工具为 `1` 本次 · `2` 本会话允许此范围 · `3` 总是允许此范围（保存） · `4` 拒绝；Bash 可额外选择命令前缀授权）；
其中 Bash 默认按具体命令记，也可按安全推导出的命令前缀记（如 `Bash(go test:*)`）；文件编辑类工具的本会话授权按编辑能力记，持久授权则写入 `Edit(<path>)` 文件路径规则；
`reames-agent run` 保持自主运行但仍然遵守 `deny`。
...
原生 Windows helper 使用 Reames Agent 内置的 Windows sandbox backend：
...
Windows 沙盒排障：沙盒会把 Reames Agent 可执行文件自身以隐藏 helper 方式重新拉起，
...
各自暴露的内容。`make build` 还会产出 `bin/reames-agent-plugin-example`——一个可直接运行的
...
**已有 Claude Code 的 `.mcp.json`？** 直接放到项目根目录，Reames Agent 会原样读取——其
`mcpServers` 规范（`command`/`args`/`env`、`type`/`url`/`headers`、`${VAR}` 展开）
与 `[[plugins]]` 字段一一对应。两处来源会合并加载；同名时以 `reames-agent.toml` 为准。
...
**从 `0.x` 升级？** 旧的 `~/.reames-agent/config.json` 仍会被读取（读其 `mcpServers`、并遵从
`mcpDisabled`），作为最低优先级来源——所以 MCP 服务器照常可用；方便时再把它们挪进
`reames-agent.toml` 的 `[[plugins]]` 或 `.mcp.json`。

## 斜杠命令

交互式 `reames-agent` 会话里，内置命令（`/compact`、`/new`、`/clear`、`/rewind`、`/tree`、`/branch`、`/switch`、`/todo`、`/model`、`/mcp`、`/skills`、`/hooks`、`/memory`、`/memory-v5`、`/goal`、`/output-style`、`/sandbox`、`/language`、`/auto-plan`、`/reasoning-language`、`/help`）在本地执行——`/help` 可列出全部。
...
是放在 `.reames-agent/commands/`（项目）或 `~/.reames-agent/commands/`（用户）下的 Markdown 文件——
`review.md` 即 `/review`，子目录构成命名空间（`git/commit.md` → `/git:commit`）。文件正文
是 prompt 模板，调用即作为一轮对话发出。

`/memory` 会同时列出记忆文档（默认 `AGENTS.md` / 兼容旧 `REASONIX.md`）和已保存的 auto-memory 条目。
...
Memory v5 在 CLI/TUI、`reames-agent serve` 和桌面端默认开启，因为这些入口共用同一套本地
controller。它会把本地、按项目隔离的执行轨迹和编译器状态写在 Reames Agent home 下，并且只有
...
`reames-agent config memory-v5 off|observe|compact|on|status`。桌面端还可以在设置 → 通用 → Memory v5 中控制。
相关 token、缓存、成本和编译器计数只保存在本地会话状态，不存在桌面聚合上报设置或上传管线。

CLI/TUI 和 `reames-agent serve` 使用同一个 user/global 配置。项目内的 `reames-agent.toml` 不能覆盖
这个 user/global 设置。CLI 命令会更新底层配置；高级用户也可以手动编辑 Reames Agent home 下的
用户配置：
...
Desktop 的 token、缓存、成本和 Memory v5 编译器计数只保存在本地会话状态。Reames Agent
不提供匿名启动、聚合指标、性能或崩溃上传服务。CLI 可以在本地轮次使用 Memory v5；使用
`reames-agent run --metrics <path>` 时，JSON 还会输出内容无关的 `memory_compiler_*` 汇总字段，
...
Goal 是长期目标的统一运行机制。普通 `/goal` 继续走轻量 Goal：Reames Agent 会持续推进，直到
...
`.reames-agent/autoresearch/<task-id>/` 任务目录。高级用户可以用
...
它会创建或复用项目级 `.reames-agent/autoresearch/<task-id>/` 目录。新任务默认使用
...
完成。动态运行态只写进 `.reames-agent/autoresearch/...`，不写入 `AGENTS.md`、兼容旧 `REASONIX.md`、
project memory、tool schema 或 cache-stable system prompt。公开发布、破坏性操作、凭证、
付款和外部通知仍然遵守正常的 approval、privacy 与 cache gate。

## @ 引用

在消息里写 `@` 引用，Reames Agent 会在发送前解析成带标签的上下文块：`@path/to/file`（或
...
`reames-agent setup` 刻意保持首次体验极简：选 provider → 输入 key（所选 provider 的所有
SKU 都会启用）。若要让两个模型协同（执行器 + 规划器，各自独立、缓存稳定的
session），向导后手动在 `reames-agent.toml` 加一行即可：
...
Planner 会看到已加载的 `AGENTS.md` / 兼容旧 `REASONIX.md` 记忆，并拿到一小组只读研究工具，
因此可以先检查相关文件再把计划交给执行器。写入类和流程类工具仍只给执行器使用。
`max_steps` 限制执行器；`planner_max_steps` 只限制规划器，两者都可设为 `0` 表示不限。

执行资源上限请放在用户级配置。项目 `./reames-agent.toml` 不会覆盖 `max_steps`、
`planner_max_steps` 或四个 `subagent_max_*` 委派树预算。
...
每次前台 `task`、`read_only_task`、`parallel_tasks` 或 subagent skill 都会新建或继承
同一棵树的共享账本。默认最多同时进行 3 个 Provider stream、全树聚合 100 个 Provider
round；token 和墙钟上限默认关闭。并发槽只在 Provider stream 活跃时持有，因此父 subagent
等待嵌套 child 时不会因占槽形成死锁。前台树继承当前 turn 的取消；后台 `task` 改为继承
session job context，可跨 turn 继续，并在 job/session teardown 时取消。Provider 只能在响应后
上报 usage，因此单次响应可能越过 `subagent_max_tokens`；运行时会记录真实总量并立即取消其余
descendants，而不会宣称能够事前精确截断。

可写 subagent 通过不进入 prompt 的 effects bridge 工作。它只把结构化 read/write/command
receipt 和可 preview 的文件变化归并到发起 turn：child 验证只有位于 child write 之后，才能满足
父任务 project checks；父 checkpoint 也可以回退 child 的可预览编辑。失败或取消的 previewed
writer 会保留为不确定 mutation boundary，要求后续重新验证，但绝不会当成成功证据。后台 task
可以跨 turn 继续；一旦新 turn 已开始，旧任务迟到的 receipt 和 checkpoint callback 会被拒绝，
避免旧任务为新回答提供伪证，此时必须从 job 结果与磁盘重新核验。Effects bridge 本身只是进程内
状态，不是可 crash-resume 的 child receipt journal。Child writer 会使 parent 旧的 durable project-check
引用失效，但 child-only bash 不在 parent transcript，仍只能作为当前 turn 证据；后续 continuation 需要
durable proof 时应由 root 重新运行检查。Root 引用只保存命令哈希与 tool-call ID，恢复时还会按 exact
transcript anchor 复验。`bash` 无法静态 preview 目标文件，因此 child `bash` 与 root `bash` 一样没有
自动逐文件 checkpoint。

可 preview 的文件 writer 执行前，Reames 必须先把 checkpoint 持久化；root turn 还会刷新
runtime sidecar，并确认 in-flight turn marker 匹配当前 session 和 turn boundary，而不是旧 turn
遗留的 marker。任一步写入失败，工具都会在接触目标文件前被拒绝，
失败原因会返回给模型；应先修复 session/checkpoint 存储可写性再重试。该门禁让进程中断后的
pre-edit bytes 可恢复，但不会把任意 shell 写入变成 crash-atomic 事务。
...
的任务会自动进入 plan mode：Reames Agent 先只读生成计划，待用户批准后才
编辑文件或执行有副作用的命令。`auto_plan_classifier` 可以指定便宜的 provider，例如
`deepseek-flash`；它只在边界输入上调用，分类失败会回退到启发式规则。也可以用
在 `reames-agent` 会话里用 `/auto-plan off|on` 修改用户级设置，或在 shell/脚本里用
`reames-agent config auto-plan off|on`。Auto-plan 只认用户级设置；项目
`reames-agent.toml` 里的 `agent.auto_plan` 会被忽略。可见思考语言也采用类似形态：
会话里用 `/reasoning-language auto|zh|en`，shell/脚本里用
`reames-agent config reasoning-language auto|zh|en`。Memory v5 使用 `/memory-v5 off|observe|compact|on|status`
或 `reames-agent config memory-v5 off|observe|compact|on|status`，并且只认用户级设置。只有明确想为

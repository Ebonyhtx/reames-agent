...
| 状态与恢复 | 部分实现 | session JSONL、lease/recovery 和 control persistence 边界继续复用同一语义。v2 runtime sidecar 持久化 Goal/Plan/Todo、continuation 安全计数、message count、transcript digest、monotonic revision，以及最新 writer epoch 的最小项目检查引用。引用仅含配置命令 SHA-256 与 root bash tool-call ID；恢复要求 exact transcript anchor，并从 transcript 验证该引用是最新 writer 后该检查最新一次成功 tool result。writer/mutation attempt、后续失败检查、新 Goal、append/divergence 均清空或拒绝旧引用；sidecar 不保存命令正文、工具输出或模型文本。所有 completion 都经过 Todo/project checks，strict 只接受实际 self-check turn。Resume/Switch/Branch/Fork/Rewind 整体替换运行态；Checkpoint fail-closed、委派共享预算、writable effects generation 边界和 `backup create/verify/restore` 继续成立 | 完整 Evidence ledger 仍是单 turn 内存态；durable 部分只覆盖配置的 root 项目检查，不是任意工具/手工声明或 child-only receipt journal。委派账本/effects bridge 不提供 crash-resume，过期后台 receipt 会丢弃；后台副作用必须从 job 产物和磁盘重新核验。Runtime/checkpoint 写失败尚未全部向 writer fail closed 传播；bash 无静态目标 preview。`AtomicWriteFile` Windows fallback、RestoreCode 路径 TOCTOU、transcript/runtime/workspace 断电事务、ACL/xattr/硬链接身份、无 lease 嵌入方单写者和跨根备份 journal 仍未关闭 |

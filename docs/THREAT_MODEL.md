...
| 状态与恢复 | 部分实现 | session JSONL、lease/recovery 和 control persistence 边界继续复用同一语义。v2 runtime sidecar 持久化 Goal/Plan/Todo、continuation 安全计数、message count、transcript digest 与 monotonic revision；相同 sidecar 路径的进程内 revision 检查/替换串行化，跨进程依赖 session lease。所有 completion 都经过 Todo/project checks，strict 只接受实际 self-check turn。Resume/Switch/Branch/Fork/Rewind 整体替换运行态；Checkpoint digest/divergence/legacy/负边界/无效 runtime fail closed，durable truncate tombstone 与 path/case/hard-link alias 回滚已覆盖。每棵 subagent 委派树共享活跃 Provider round、聚合 step/token/time 和 root cancellation 账本；Provider 在错误前已上报的 usage 也计入。可写 child 只向祖先传播结构化 receipt、父 tool-call provenance 和 pre-edit callback，partial failure/cancel 保留 mutation boundary；evidence generation 与 turn-scoped checkpoint callback 拒绝迟到后台效果污染新 turn。`backup create/verify/restore` 继续提供分根、哈希、源身份、portable path 和仅新目标恢复门禁 | 委派账本/effects bridge 是进程内 runtime 状态，不提供 crash-resume；Provider 只在响应后报告 usage，因此单个响应可能越过 token 上限，越界后才取消整树。Evidence ledger 仍是单 turn 内存态，过期后台 receipt 会 fail closed 丢弃而不是成为 durable proof；后台文件副作用只能从 job 产物和磁盘重新核验。Runtime/checkpoint 写失败尚未全部向 writer fail closed 传播；bash 没有静态目标 preview，和 root Agent 一样不能自动做逐文件 checkpoint。`AtomicWriteFile` 的 Windows cross-device fallback 可能原地复制；RestoreCode 路径复查后仍按路径写入，存在需要 handle-relative no-reparse/resolve-beneath 关闭的 TOCTOU；transcript/runtime/workspace 不构成断电事务，ACL/xattr/硬链接身份也不恢复。session lease 缺失的非标准嵌入方不获得跨进程 sidecar 单写者保证。备份仍可能含 session/memory/config secret，内嵌 hash 只证明自洽，`--offline` 不是全局锁，Windows 依赖目标 ACL，跨根恢复没有 durable crash journal |

...
2. **扩展供应链**：权限 manifest、内容摘要、默认禁用和 package-owned 进程 sandbox 降低了静默漂移与宿主密钥泄露风险，但插件、用户 MCP/Hook 和 LSP 仍能执行本机代码；在签名、provenance 和运营 registry 完成前，只应安装固定 revision、可审计来源。

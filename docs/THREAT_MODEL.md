...
| 状态与恢复 | 部分实现 | session JSONL、lease/recovery 和 control persistence 边界继续复用同一语义。v2 runtime sidecar 持久化 Goal/Plan/Todo、continuation 安全计数、transcript anchor/revision 和最新 writer epoch 的最小项目检查引用；previewable writer 的 checkpoint/runtime/in-flight 任一持久化失败均在执行前拒绝。持久 subagent 在 Provider、tool 与 compaction 安全边界保存 transcript，recoverable job 只有 running metadata 成功落盘后才启动；冷启动把遗留 running ref/job 转为 `interrupted` tombstone，保留部分 log 并提供 `continue_from`，显式续跑要求先核验 workspace，不自动重放工具。Compacted transcript 的 interrupted continuation 与 memory recall 的 score/path/snippet、关闭、归档删除和稳定 system prefix 均有统一测试。所有 Goal completion 仍经过 Todo/project checks，Resume/Switch/Branch/Fork/Rewind 整体替换运行态 | 完整 Evidence ledger 仍是单 turn 内存态；durable 部分只覆盖 root 项目检查和 subagent transcript/job tombstone，不是任意工具、child-only receipt 或 effects journal。崩溃中的工具可能未执行、部分执行或已完成，系统不提供 arbitrary side effect exactly-once；后台副作用必须从 job 产物、磁盘和外部系统重新核验。`bash`/opaque writer 没有逐文件 checkpoint；`AtomicWriteFile` Windows fallback、handle-relative no-reparse、transcript/runtime/workspace 跨资源事务、ACL/xattr/硬链接身份、无 lease 嵌入方单写者和跨根备份 journal 仍未关闭 |

...
| 状态与恢复 | 部分实现 | session JSONL、lease/recovery 和 control persistence 边界继续复用同一语义。v2 runtime sidecar 持久化 Goal/Plan/Todo、continuation 安全计数、message count、transcript digest、monotonic revision，以及最新 writer epoch 的最小项目检查引用。引用仅含配置命令 SHA-256 与 root bash tool-call ID；恢复要求 exact transcript anchor，并从 transcript 验证该引用是最新 writer 后该检查最新一次成功 tool result。writer/mutation attempt、后续失败检查、新 Goal、append/divergence 均清空或拒绝旧引用；sidecar 不保存命令正文、工具输出或模型文本。Previewable writer 的 checkpoint allocation/record 或祖先 callback 失败会在执行前拒绝；root writer 还必须在同一 session handoff 临界区成功刷新 runtime sidecar，并确认 in-flight marker 匹配当前 session、message boundary 与 `preserveUser`，旧 turn marker 不被接纳。失败 Snapshot 回滚内存状态，持久 pre-edit record 可在模拟进程消失后由新 Store 恢复部分写入。所有 completion 都经过 Todo/project checks，strict 只接受实际 self-check turn。Resume/Switch/Branch/Fork/Rewind 整体替换运行态；委派共享预算、writable effects generation 边界和 `backup create/verify/restore` 继续成立 | 完整 Evidence ledger 仍是单 turn 内存态；durable 部分只覆盖配置的 root 项目检查，不是任意工具/手工声明或 child-only receipt journal。委派账本/effects bridge 不提供 crash-resume，过期后台 receipt 会丢弃；后台副作用必须从 job 产物和磁盘重新核验。`bash` 与不实现 Previewer 的外部 writer 没有逐文件 checkpoint 门禁；持久快照支持恢复不等于自动恢复或跨资源断电原子性。`AtomicWriteFile` Windows fallback、RestoreCode 路径 TOCTOU、transcript/runtime/workspace 断电事务、ACL/xattr/硬链接身份、无 lease 嵌入方单写者和跨根备份 journal 仍未关闭 |

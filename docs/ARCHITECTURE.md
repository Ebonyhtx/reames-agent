...
OpenAI 的 wire API 由 `ProviderEntry.api_mode` 显式选择：空值/`chat_completions` 保持历史兼容
协议，`responses` 使用独立 item/event transport；模型名和 URL 不触发静默切换。OpenAI Responses
保留向后兼容的 `reasoning.encrypted_content` include（当前 `store=false` API 默认也会返回），将 opaque
reasoning item 保存在 `provider.ReasoningBlocks` 并在
`store=false` 的工具续轮回放；Anthropic Messages 同样按顺序保存 signed thinking 与 opaque
`redacted_thinking`。两类 opaque Data 均不进入展示 DTO 或导出。两条协议仍输出统一
`provider.Chunk`，Agent loop 不感知 GPT/Claude 专用前端或 transport。

### Desktop MCP 与插件生命周期

Desktop 的 MCP Host/Registry 是跨 tab 共享的 capability surface，任何 add/update/remove/reconnect/reverify/
clear-auth/reader-trust/enable/tier 或插件 install/update/remove/enable 不能只检查 active tab。统一锁序为：

```text
pluginRuntimeGate writer
  -> runtimeBuildGate writer
  -> runtimeRebuildMu
  -> every visible + detached Controller.BeginRuntimeMutation
  -> App.mu / shared plugin.Host / controller Registry / config edit lock
```

`pluginRuntimeGate` 阻止新的 Desktop turn、shell、compact 和 session rotation 在 mutation 中途取得 Controller；
Controller 的 `BeginRuntimeMutation` 再原子关闭“检查 idle → 进入 Run/rotate”的窗口，并把 pending prompt 与
background job 视为 busy。`runtimeBuildGate` 只负责异步 controller build admission：writer 取得后作废所有已经
按旧 generation 启动但尚未发布的 build，mutation 结束并释放 writer 后才重启空 tab。同步设置 rebuild 不经
该 read wrapper，避免与 `runtimeRebuildMu` 形成反向等待。

插件 package mutation 在执行文件/manifest 变化前作废旧 build；MCP mutation 在读取或修改 shared Host 前完成
同一动作。这样 plugin uninstall、trust/reverify、disable/reconnect 与 sibling/detached turn 不能交错，晚到的
...
`internal/bot` 的 CLI foreground Gateway 与 Desktop bot runtime 共用
`<Reames Agent home>/bot/delivery-ledger.json`。schema v2 把入站 durable claim、远端连续 cursor 和
outbound final-response obligation 放在同一份 0600、原子替换、有界 JSON 状态中；同路径长生命周期
OS 文件锁保证一个进程只能有一个 writer，第二个 Gateway fail closed。旧 schema v1 在持锁后原子迁移，
损坏、未知版本、身份不一致、4 MiB 总大小超限或持久化失败均阻断处理。

```text
inbound durable claim
  -> Controller.RunTurn (only when no existing obligation owns the claim)
  -> render successful final text into stable semantic chunks
  -> persist obligation.pending (identity + target + final text chunks)
  -> persist obligation.attempting before each platform Send
  -> platform ACK
  -> persist next_chunk, or on the last chunk atomically:
       delete obligation
       mark every constituent inbound claim delivered
       advance each contiguous recovery checkpoint / Telegram offset
```

每个 obligation 最多 1 MiB、512 个纯文本分片；审批、Ask、progress、card 和 media 不进入 obligation。
`pending` 表示尚无发送尝试，重启可直接发送且不显示警告；`attempting` 表示进程可能在平台 ACK 与本地
commit 之间退出，`failed` 表示平台发送没有得到确认，两者恢复时都只在首个重发分片添加可见“可能重复”
标记。已经持久 ACK 的前置分片不会重发，只从 `next_chunk` 继续。重复 inbound 若命中 obligation，Gateway
直接恢复原文本，不装配 Controller、不调用 Provider，因此模型不会为了重建答复再次运行。

obligation 必须保存最终答复文本才能在重启后原样恢复，但正文只存在本地 0600 ledger：日志、adapter

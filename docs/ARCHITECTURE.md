...
迁移按纵向路径进行：命令、event/display DTO、主要会话 persistence 路径、Desktop system-prompt-aware rebuild、settings provider-kind view、展示安全 memory suggestions、Serve title provider 装配与 ACP metadata title 已经收口；Serve/Bot/ACP service 已无 runtime 直连。下一步迁移 Desktop app/tab 与 CLI/ACP composition root 剩余装配/渲染边界。CLI/Bot/ACP 为拥有 turn 生命周期而保留同步 `RunTurn`，它与异步 command acknowledgement 是不同语义，不强行合并成伪统一接口。

...
完整 evidence ledger 仍是单 turn、内存态收据。Board 可以显示当前收据数、写入/命令活动和触及路径，但这些摘要不会进入 provider prompt，也不是跨进程 durable proof。跨 continuation 只持久化项目检查的最小引用：配置命令的 SHA-256 与 root bash tool-call ID。writer 或 mutation attempt 开启新 epoch 并清空旧引用；同一检查的最新失败覆盖旧成功。恢复时只有 sidecar 的 transcript count/digest 精确匹配，并且该 ID 能解析到最新 writer 之后该检查最新一次成功 tool result，引用才生效。命令正文、工具输出、模型文本、任意手工证据和 child-only receipt 不进入这组引用。
...
- `internal/agent/delegation_budget.go`、`subagent_effects.go`：整树资源账本与 prompt 不可见 effects 归并

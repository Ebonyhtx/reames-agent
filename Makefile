...
build:
	CGO_ENABLED=0 go build -ldflags "$(LDFLAGS)" -o bin/reames-agent$(GOEXE) ./cmd/reames-agent
	CGO_ENABLED=0 go build -ldflags "$(LDFLAGS)" -o bin/reames-agent-plugin-example$(GOEXE) ./cmd/reames-agent-plugin-example
...
cross:
	...
	@for p in darwin/amd64 darwin/arm64 linux/amd64 linux/arm64 windows/amd64 windows/arm64; do \
		...
		echo "build $$os/$$arch"; \
		CGO_ENABLED=0 GOOS=$$os GOARCH=$$arch go build -ldflags "$(LDFLAGS)" -o dist/reames-agent-$$os-$$arch$$ext ./cmd/reames-agent; \
	done
